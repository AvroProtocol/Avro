...
/**
 * Cross-chain bridging via Relay (https://relay.link) with a AVRO rebate on the
 * relayer spread.
 *
...
/** Chains Avro bridges between. All four are Relay-supported. */
...
/** AVRO (Avro), 18 decimals. Deployed on Robinhood Chain only. */
export let AVRO_ADDRESS = "" as Address;
export const AVRO_DECIMALS = 18;
export const AVRO_SYMBOL = "AVRO";

/** Configure the verified deployed $AVRO contract address for SDK consumers. */
export function configureAvroTokenAddress(address: Address) {
  if (!/^0x[0-9a-fA-F]{40}$/.test(address)) {
    throw new Error("Invalid $AVRO contract address");
  }
  AVRO_ADDRESS = address;
}
...
/**
 * Converts a micro-USD rebate into AVRO wei at a given AVRO/USD price.
 *
 * `avroUsdPrice` is a decimal string ("0.0184"). Returns 0 when the price is
 * unavailable or non-positive rather than dividing by zero - callers should
 * treat a zero result as "not claimable yet".
 */
export function rebateMicrosToAvroWei(rebateMicros: bigint, avroUsdPrice: string | number): bigint {
  const priceMicros = usdToMicros(avroUsdPrice);
  if (rebateMicros <= 0n || priceMicros <= 0n) return 0n;
  return (rebateMicros * 10n ** BigInt(AVRO_DECIMALS)) / priceMicros;
}

export function formatAvro(weiAmount: bigint, displayDecimals = 4): string {
  const base = 10n ** BigInt(AVRO_DECIMALS);
  const whole = weiAmount / base;
  const fraction = (weiAmount % base).toString().padStart(AVRO_DECIMALS, "0").slice(0, displayDecimals);
 ...
 *
 * `appFeeRecipient` / `appFeeBps` attach Avro's own fee to the route; that
