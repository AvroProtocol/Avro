...
export class AvroWallet {
  ...
  constructor(config: AvroWalletConfig) {
  ...
  /**
   * Onboard and register a new 2-of-3 threshold smart account with the live AVRO co-signer
   */
  static async create(options?: {
  ...
  }): Promise<{ wallet: AvroWallet; shardC: LocalShard }> {
    ...
    const reg = (await response.json()) as AvroWalletRegistration;
  ...
  /**
   * Combine two 65-byte signatures into the 130-byte payload expected by AvroAccount.sol
   */
  ...
  async signUserOp(userOpHash: Hex): Promise<Hex> {
    ...
    return AvroWallet.combineSignatures(sigA, sigB);
  }
  ...
  static async recoverWallet(options: RecoveryOptions): Promise<BundlerSubmissionResponse> {
    ...
    const signature = AvroWallet.combineSignatures(sigC, signatureB);
