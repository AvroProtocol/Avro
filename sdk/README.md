...
Viem-native TypeScript SDK enabling developers to embed AVYRO PROTOCOL 2-of-3 threshold self-custody directly into applications on Robinhood Chain (Chain ID: 4663).
...
```bash
bun add @avyro/protocol-sdk viem
# or
...
Generate a device key locally and register a new threshold smart account with the live AVYRO PROTOCOL co-signer:

```typescript
import { AvyroWallet, LocalShard } from "@avyro/protocol-sdk";

// 1. Generate local device key (Shard A)
const shardA = LocalShard.create("device");

// 2. Register wallet with the live AVYRO PROTOCOL co-signer
const { wallet, shardC } = await AvyroWallet.create({
...
const receipt = await AvyroWallet.recoverWallet({
...
console.log("PayLink URL:", `https://avyroprotocol.com/pay/${paylink.slug}`);
...
console.log("Factory:", AVYRO_FACTORY_ADDRESS);
```
