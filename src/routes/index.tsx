...
function shortAddress(address: string) {
  if (!address) return "CA publishes from config.json";
      ...
      <main>
        ...
        <section className="nx-section nx-developers" id="developers">
          <div className="nx-container nx-dev-layout">
            <div className="nx-dev-copy" data-reveal>
              ...
              <a className="nx-button nx-button-primary" href="/docs.html">Read the SDK docs <ArrowUpRight size={15}/></a>
            </div>
        ...
        <section className="nx-section nx-mobile-home" id="mobile">
          <div className="nx-container nx-mobile-home-layout">
            <div className="nx-mobile-home-copy" data-reveal>
              ...
              <div className="nx-mobile-home-actions"><a className="nx-button nx-button-light" href={avyroApiUrl("v1/downloads/android")}><Download size={15}/> Download Android</a><a className="nx-button nx-button-outline-dark" href="#security">Security model <ArrowRight size={15}/></a></div>
            </div>
        ...
        <section className="nx-section nx-token" id="token">
          <div className="nx-container nx-token-panel" data-reveal>
            <div className="nx-token-title"><span className="nx-index nx-index-dark">08 / ECOSYSTEM</span><h2><small>$</small>{config.ticker}</h2><p>Launch access and the canonical contract address stay synced from <code>config.json</code>. Publish the CA once and the buy destination updates automatically.</p></div>
            <div className="nx-token-live">
              <div className="nx-ca-label"><span><i className={config.contractAddress ? "is-live" : ""}/>CONTRACT ADDRESS</span><b>{config.contractAddress ? "LIVE" : "AWAITING DEPLOYMENT"}</b></div>
              <button type="button" className="nx-ca-box" onClick={copyCa} disabled={!config.contractAddress} title={config.contractAddress || "Contract address not published yet"}><span>{shortAddress(config.contractAddress)}</span>{config.contractAddress && (copied ? <Check size={16}/> : <Copy size={16}/>)}</button>
              <div className="nx-token-actions"><a href="/avyro" className="nx-button nx-button-light">Token page <ArrowRight size={15}/></a>{buyUrl ? <a href={buyUrl} target="_blank" rel="noreferrer" className="nx-button nx-button-outline-dark">Buy ${config.ticker} <ArrowUpRight size={15}/></a> : <span className="nx-button nx-button-outline-dark is-disabled">Buy opens when CA is live</span>}</div>
              <small className="nx-buy-preview">BUY ROUTE · {buyUrl || "https://www.ponsfamily.com/launchpad/{ca}"}</small>
            </div>
          </div>
        </section>
