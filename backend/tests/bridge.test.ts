...
describe("Staking reserve accounting", () => {
  it("scales decimal AVYRO amounts to wei without floating point drift", () => {
    ...
    expect(avyrDecimalToWei("")).toBe(0n);
  });
  ...
  it("treats staked AVYRO as a reserve rebates may not spend", () => {
    ...
    expect(available).toBe(avyrDecimalToWei("30000"));
    // A 50k rebate must be refused even though the pool "has" 150k.
    expect(avyrDecimalToWei("50000") > available).toBe(true);
  });

  it("reports underfunded when stake plus promised rebates exceed the balance", () => {
    const balance = avyrDecimalToWei("100000");
    const staked = avyrDecimalToWei("95000");
    const pendingRebates = avyrDecimalToWei("10000");
