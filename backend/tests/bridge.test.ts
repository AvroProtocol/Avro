...
import { avroDecimalToWei } from "../src/treasury.js";
import { AVRO_TOKEN_ADDRESS, POOL_ADDRESS, assertAvroTokenConfigured } from "../src/poolWallet.js";
import {
  deviationBps,
...
describe("Bridge rebate math", () => {
  ...
  it("converts a USD rebate into AVRO wei at the given price", () => {
    // $0.25 of rebate at $0.05/AVRO = 5 AVRO
    expect(microsToAvroWei(250_000n, "0.05")).toBe(5n * 10n ** 18n);
  });

  it("returns zero AVRO when the price is missing or zero", () => {
    expect(microsToAvroWei(250_000n, "0")).toBe(0n);
    expect(microsToAvroWei(250_000n, "")).toBe(0n);
  });
});
...
describe("Bridge API", () => {
  it("advertises the four supported chains and programme terms", async () => {
    ...
    expect(res.body.rebateCurrency.symbol).toBe("AVRO");
    expect(res.body.rebateCurrency.address).toBe(AVRO_TOKEN_ADDRESS);
  });
...
describe("AVRO price guard", () => {
  // AVRO has ~$4.8k of liquidity, so spot is cheap to move. A lower price pays
  // out MORE AVRO per USD, so a claimant is incentivised to push it down.
  ...
  it("pays more AVRO when the price is pushed down - the reason for the guard", () => {
    const rebate = 250_000n; // $0.25
    const atReference = microsToAvroWei(rebate, "0.000020");
    const atHalfPrice = microsToAvroWei(rebate, "0.000010");
...
describe("Pool wallet configuration", () => {
  it("never aliases AVRO to USDG", () => {
    if (AVRO_TOKEN_ADDRESS) {
      expect(AVRO_TOKEN_ADDRESS.toLowerCase()).not.toBe("0x5fc5360d0400a0fd4f2af552add042d716f1d168");
    }
  });
  ...
  it("requires a verified configured AVRO token before token operations", () => {
    if (AVRO_TOKEN_ADDRESS) {
      expect(() => assertAvroTokenConfigured()).not.toThrow();
    } else {
      expect(() => assertAvroTokenConfigured()).toThrow(/not configured/i);
    }
  });
});

describe("Staking reserve accounting", () => {
  it("scales decimal AVRO amounts to wei without floating point drift", () => {
    expect(avroDecimalToWei("1")).toBe(10n ** 18n);
    expect(avroDecimalToWei("10000.5000")).toBe(10000n * 10n ** 18n + 5n * 10n ** 17n);
    expect(avroDecimalToWei(0)).toBe(0n);
    expect(avroDecimalToWei("")).toBe(0n);
  });

  it("does not lose precision on amounts that break Number", () => {
    // 1,000,000 AVRO in wei exceeds Number.MAX_SAFE_INTEGER.
    expect(avroDecimalToWei("1000000")).toBe(1_000_000n * 10n ** 18n);
    expect(avroDecimalToWei("1000000") > BigInt(Number.MAX_SAFE_INTEGER)).toBe(true);
  });

  it("treats staked AVRO as a reserve rebates may not spend", () => {
    // The pool holds stake + surplus; only the surplus funds rebates.
    const balance = avroDecimalToWei("150000");
    const stakedReserve = avroDecimalToWei("120000");
    const available = balance - stakedReserve;

    expect(available).toBe(avroDecimalToWei("30000"));
    // A 50k rebate must be refused even though the pool "has" 150k.
    expect(avroDecimalToWei("50000") > available).toBe(true);
  });

  it("reports underfunded when stake plus promised rebates exceed the balance", () => {
    const balance = avroDecimalToWei("100000");
    const staked = avroDecimalToWei("95000");
    const pendingRebates = avroDecimalToWei("10000");
