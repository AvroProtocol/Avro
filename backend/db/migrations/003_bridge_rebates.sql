...
-- accrued in micro-USD and paid out in AVRO on Robinhood Chain.
...
CREATE TABLE IF NOT EXISTS bridge_rebates (
  ...
  -- Fee figures are captured server-side from the quote Avro itself issued,
...
CREATE TABLE IF NOT EXISTS rebate_claims (
  ...
  -- AVRO owed, fixed at claim time using the price below.
  avro_wei NUMERIC(78, 0) NOT NULL,
  avro_usd_price NUMERIC(38, 18) NOT NULL,
  ...
  CONSTRAINT rebate_claims_positive CHECK (amount_micros_usd > 0 AND avro_wei > 0)
);
