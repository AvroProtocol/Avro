 ...
 *
 * One wallet backs both products: it custodies AVRO staked for gasless
 * transactions and pays ETH gas sponsorship, and from v0.1.8 it also settles
 * bridge rebates. Because staked AVRO is user property owed back on unstake,
...
/** AVRO (Avro), 18 decimals, Robinhood Chain. */
export const AVRO_TOKEN_ADDRESS = (process.env.AVRO_TOKEN_ADDRESS?.trim() || "") as Address;

export const AVRO_DECIMALS = 18;

/** USDG - not AVRO. Kept here so misconfiguration can be detected explicitly. */
...
/**
 * Guards against the AVRO token address being set to USDG.
 *
 ...
 */
export function assertAvroTokenConfigured(): void {
  if (!/^0x[0-9a-fA-F]{40}$/.test(AVRO_TOKEN_ADDRESS)) {
    throw new Error(
      "AVRO_TOKEN_ADDRESS is not configured. Set the verified deployed $AVRO contract address before enabling token operations."
    );
  }
  if (AVRO_TOKEN_ADDRESS.toLowerCase() === USDG_ADDRESS) {
    throw new Error("AVRO_TOKEN_ADDRESS must not be the USDG contract.");
  }
}
...
/** AVRO held by the pool, in wei. */
export async function getPoolAvroBalance(): Promise<bigint> {
  assertAvroTokenConfigured();
