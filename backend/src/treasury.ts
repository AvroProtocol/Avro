...
export async function getPoolSolvency(): Promise<PoolSolvency> {
  ...
  return {
    ...
    avyrBalanceWei: balanceWei?.toString() ?? null,
...
async function recordAttempt(
  ...
  await pool.query(
    `INSERT INTO rebate_payout_attempts (claim_id, treasury_address, avyr_wei, tx_hash, outcome, error)
     VALUES ($1,$2,$3,$4,$5,$6)`,
    [claimId, POOL_ADDRESS, avyrWei.toString(), txHash ?? null, outcome, error ?? null]
  );
}
...
export async function processPendingPayouts(
  ...
  const candidates = await pool.query(
    `SELECT id, user_address, avyr_wei FROM rebate_claims
  ...
  for (const claim of candidates.rows) {
    const avyrWei = BigInt(claim.avyr_wei);
    const recipient = claim.user_address as Address;

    if (!isAddress(recipient)) {
      await recordAttempt(claim.id, avyrWei, "failed", undefined, "Invalid recipient");
      results.push({ claimId: claim.id, recipient, avyrWei: claim.avyr_wei, outcome: "failed", error: "Invalid recipient" });
      continue;
    }

    if (avyrWei <= 0n || avyrWei > MAX_PAYOUT_WEI) {
      const error = `Payout of ${avyrWei} wei is outside the permitted range`;
      await recordAttempt(claim.id, avyrWei, "failed", undefined, error);
      results.push({ claimId: claim.id, recipient, avyrWei: claim.avyr_wei, outcome: "failed", error });
      continue;
    }
    ...
    if (dryRun) {
      await recordAttempt(claim.id, avyrWei, "dry_run");
      results.push({ claimId: claim.id, recipient, avyrWei: claim.avyr_wei, outcome: "dry_run" });
      spendable -= avyrWei;
      continue;
    }
    ...
    if (!locked) {
      results.push({ claimId: claim.id, recipient, avyrWei: claim.avyr_wei, outcome: "skipped" });
      continue;
    }
    ...
    try {
      ...
      await recordAttempt(claim.id, avyrWei, "sent", txHash);
      ...
      if (succeeded) spendable -= avyrWei;

      results.push({
        claimId: claim.id,
        recipient,
        avyrWei: claim.avyr_wei,
      ...
      await recordAttempt(claim.id, avyrWei, "failed", undefined, message);
      results.push({ claimId: claim.id, recipient, avyrWei: claim.avyr_wei, outcome: "failed", error: message });
    }
  }
