...
/** Avyro's own fee on each route, in basis points. Funds the rebate pool. */
...
export async function resolveAvyrPrice(): Promise<AvyrPriceResult> {
  ...
  const spot = await fetchAvyrSpotPrice();
  if (!spot) {
...
export async function fetchAvyrUsdPrice(): Promise<string> {
  return (await resolveAvyrPrice()).price;
}
