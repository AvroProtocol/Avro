 ...
 * out in AVRO on Robinhood Chain, since AVRO exists only there.
 *
...
bridgeRouter.get("/v1/bridge/chains", (_req: Request, res: Response) => {
  res.status(200).json({
    ...
    rebateCurrency: { symbol: "AVRO", address: AVRO_ADDRESS, decimals: AVRO_DECIMALS },
...
bridgeRouter.get("/v1/bridge/rebates/:address", async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const avroPrice = (await resolveAvroPrice()).price;
    ...
    res.status(200).json({
      ...
      claimableAvro:
        Number(avroPrice) > 0 ? microsToAvroWei(claimable, avroPrice).toString() : null,
      avroUsdPrice: Number(avroPrice) > 0 ? avroPrice : null,
 ...
 *
 * Creates the ledger entry and attaches the accruals atomically, fixing the AVRO
...
bridgeRouter.post("/v1/bridge/rebates/:address/claim", async (req: Request, res: Response): Promise<void> => {
  ...
  const priced = await resolveAvroPrice();
  if (priced.rejection || !(Number(priced.price) > 0)) {
    res.status(503).json({
      error: priced.rejection || "AVRO price is unavailable, so a claim cannot be priced right now",
  ...
  try {
    ...
    const avroWei = microsToAvroWei(totalMicros, avroPrice);
    const claim = await client.query(
      `INSERT INTO rebate_claims (
         user_address, amount_micros_usd, avro_wei, avro_usd_price, settlement_chain_id,
         avro_price_source, avro_reference_price, status
    ...
    res.status(201).json({
      ...
      avroWei: avroWei.toString(),
      avroUsdPrice: avroPrice,
      avroPriceSource: priced.source,
      avroToken: { symbol: "AVRO", address: AVRO_ADDRESS, decimals: AVRO_DECIMALS },
      settlementChainId: SETTLEMENT_CHAIN_ID,
...
bridgeRouter.get("/v1/bridge/claims/:address", async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const result = await query(
      `SELECT id, amount_micros_usd, avro_wei, avro_usd_price, status, tx_hash,
    ...
    res.status(200).json({
      ...
      claims: result.rows.map((row) => ({
        ...
        avroWei: row.avro_wei,
        avroUsdPrice: row.avro_usd_price,
        status: row.status,
        txHash: row.tx_hash,
...
/**
 * Treasury health: AVRO on hand versus AVRO already promised.
 *
