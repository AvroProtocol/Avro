...
import { avroDecimalToWei } from "../treasury.js";
...
const avroTokenAddress = AVRO_TOKEN_ADDRESS;
...
stakingRouter.get("/v1/staking/config", (_req: Request, res: Response) => {
  res.status(200).json({
    ...
    tokenAddress: avroTokenAddress,
    minStake: 10000,
    cycleDays: 30,
    tiers: [
      { name: "Tier 1", minAvro: 10000, txnsPerMonth: 25 },
      { name: "Tier 2", minAvro: 50000, txnsPerMonth: 100 },
      { name: "+50k Steps", minAvro: 50000, txnsPerMonth: "+75 txns for each 50k" },
      { name: "Tier Unlimited", minAvro: 1000000, txnsPerMonth: "Unlimited" },
    ],
  });
});
...
// 3. Record a Stake after User transfers $AVRO to Platform Pool
stakingRouter.post("/v1/staking/stake", async (req: Request, res: Response): Promise<void> => {
  ...
  if (isNaN(numAmount) || numAmount < 10000) {
    res.status(400).json({
      error: "Minimum stake is 10,000 $AVRO to activate gasless transactions",
...
stakingRouter.post("/v1/staking/renew", async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (stakedAmount < 10000) {
      await client.query("ROLLBACK");
      res.status(400).json({
        error: "Cannot renew: staked balance is below 10,000 $AVRO minimum",
...
// 5. Unstake $AVRO and Return from Platform Pool Wallet
stakingRouter.post("/v1/staking/unstake", async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (numAmount > currentStaked) {
      await client.query("ROLLBACK");
      res.status(400).json({
        error: `Cannot unstake ${numAmount} $AVRO. Currently staked: ${currentStaked} $AVRO`,
        code: "EXCEEDS_STAKED_BALANCE",
      });
      return;
    }

    // The stake is debited here and the AVRO transfer is broadcast after this
    ...
    const unstakeWei = avroDecimalToWei(numAmount);
...
/**
 * Sends unstaked AVRO back from the pool wallet.
 *
...
async function settleUnstake(
  ...
  const fail = async (error: string) => {
    try {
      ...
      // Give the stake back; the user still holds it until AVRO actually moves.
      await pool.query(
  ...
  if (!signer) {
    return fail("Pool wallet signer is not configured, so AVRO could not be returned");
  }

  // The pool also custodies other stakers' AVRO, so a transfer that would
  // overdraw it is refused up front rather than broadcast to revert.
  try {
    const balance = await getPoolAvroBalance();
    if (balance < amountWei) {
      return fail(`Pool holds ${balance} AVRO wei, which is less than the ${amountWei} owed`);
    }
  } catch (err) {
    return fail(`Could not read pool AVRO balance: ${(err as Error).message}`);
  }
