...
const GITHUB_REPO_OWNER = process.env.GITHUB_REPO_OWNER || "AvroProtocol";
const GITHUB_REPO_NAME = process.env.GITHUB_REPO_NAME || "Avro";

// Primary actions repository where GitHub Actions runs execute and build artifacts are stored
const ACTIONS_REPO_OWNER = process.env.ACTIONS_REPO_OWNER || "AvroProtocol";
const ACTIONS_REPO_NAME = process.env.ACTIONS_REPO_NAME || "Avro";
 ...
 * checks the configured Actions repository first, then the public Avro repository.
 */
async function fetchGithubReleases(): Promise<ReleaseResponse[]> {
  const token = getGitToken();
  const headers: Record<string, string> = {
    Accept: "application/vnd.github+json",
    "User-Agent": "Avro-Backend",
  };
  if (token) {
    ...
    try {
      ...
      if (p === "android" || p === "apk" || p === "mobile") {
        match = artifacts.find(
          (a) =>
            a.name === "avro-mobile-apk" ||
            a.name === "avro-mobile" ||
            (a.name.includes("mobile") && a.name.includes("apk"))
        );
      } else if (p === "windows" || p === "win" || p === "exe" || p === "msi") {
        match = artifacts.find((a) => a.name === "avro-windows-x64");
      } else if (p === "macos" || p === "mac" || p === "darwin" || p === "dmg") {
        match = artifacts.find((a) => a.name === "avro-macos-arm64");
      } else if (p === "linux" || p === "deb" || p === "appimage") {
        match = artifacts.find((a) => a.name === "avro-linux-x64");
      }
  ...
  try {
    ...
    if (release) {
      ...
      res.status(200).json({
        ...
        releaseName: release.name || "Avro Protocol",
        publishedAt: release.published_at,
        downloads: {
          windows: {
            platform: "windows",
            name: "Windows (x64 Setup)",
            filename: winAsset?.name || "AVRO_0.1.0_x64-setup.exe",
          ...
          macos: {
            ...
            filename: macAsset?.name || "AVRO_aarch64.app.tar.gz",
          ...
          linux: {
            ...
            filename: linuxAsset?.name || "AVRO_0.1.0_amd64.deb",
          ...
          android: {
            platform: "android",
            name: "Android (APK)",
            filename: apkAsset?.name || "avro-mobile.apk",
            size: androidSize,
    ...
    res.status(200).json({
      version: "v0.1.0",
      releaseName: "Avro Protocol",
      downloads: {
        windows: {
          platform: "windows",
          name: "Windows (x64 Setup)",
          filename: "AVRO_0.1.0_x64-setup.exe",
          url: "/v1/downloads/windows",
        },
        macos: {
          platform: "macos",
          name: "macOS (Apple Silicon)",
          filename: "AVRO_aarch64.dmg",
          url: "/v1/downloads/macos",
        },
        linux: {
          platform: "linux",
          name: "Linux (x64)",
          filename: "AVRO_0.1.0_amd64.deb",
          url: "/v1/downloads/linux",
        },
        android: {
          platform: "android",
          name: "Android (APK)",
          filename: "avro-mobile.apk",
          url: "/v1/downloads/android",
        },
      },
    });
...
downloadsRouter.get("/v1/downloads/mobile/latest", async (_req: Request, res: ExpressResponse): Promise<void> => {
  try {
    ...
    if (match) {
      ...
      res.status(200).json({
        version: release.tag_name || "v0.1.0",
        releaseName: release.name || "Avro Protocol Mobile",
        publishedAt: release.published_at,
        android: {
          platform: "android",
          name: "Android (APK)",
    ...
    // Fallback to GitHub Actions build artifact on AvroProtocol/Avro
    const artMatch = await fetchLatestArtifact("android");
    if (artMatch) {
      const downloadUrl = await getArtifactDownloadUrl(artMatch.artifact, artMatch.repo);
      res.status(200).json({
        version: "v0.1.0",
        releaseName: "Avro Protocol Mobile",
    ...
    res.status(200).json({
      version: "v0.1.0",
      releaseName: "Avro Protocol Mobile for Android",
...
const FALLBACK_DIRECT_URLS: Record<string, string> = {
  windows: `${RELEASE_BASE}/Avro.Protocol_${FALLBACK_APP_VERSION}_x64-setup.exe`,
  win: `${RELEASE_BASE}/Avro.Protocol_${FALLBACK_APP_VERSION}_x64-setup.exe`,
  exe: `${RELEASE_BASE}/Avro.Protocol_${FALLBACK_APP_VERSION}_x64-setup.exe`,
  msi: `${RELEASE_BASE}/Avro.Protocol_${FALLBACK_APP_VERSION}_x64_en-US.msi`,
  macos: `${RELEASE_BASE}/Avro.Protocol_${FALLBACK_APP_VERSION}_aarch64.dmg`,
  mac: `${RELEASE_BASE}/Avro.Protocol_${FALLBACK_APP_VERSION}_aarch64.dmg`,
  darwin: `${RELEASE_BASE}/Avro.Protocol_${FALLBACK_APP_VERSION}_aarch64.dmg`,
  dmg: `${RELEASE_BASE}/Avro.Protocol_${FALLBACK_APP_VERSION}_aarch64.dmg`,
  linux: `${RELEASE_BASE}/Avro.Protocol_${FALLBACK_APP_VERSION}_amd64.AppImage`,
  deb: `${RELEASE_BASE}/Avro.Protocol_${FALLBACK_APP_VERSION}_amd64.deb`,
  appimage: `${RELEASE_BASE}/Avro.Protocol_${FALLBACK_APP_VERSION}_amd64.AppImage`,
};
...
downloadsRouter.get("/v1/downloads/:platform", async (req: Request, res: ExpressResponse): Promise<void> => {
  ...
  try {
    ...
    // 2. Fallback: Check workflow run artifacts from GitHub Actions on AvroProtocol/Avro
    if (token) {
    ...
    if (rawPlatform === "android" || rawPlatform === "apk") {
      res.status(503).json({
        error: "Android APK is being prepared for the current AVRO release.",
