    ...
    try {
      ...
      if (p === "android" || p === "apk" || p === "mobile") {
        match = artifacts.find(
          (a) =>
            a.name === "avyro-mobile-apk" ||
            a.name === "avyro-mobile" ||
            (a.name.includes("mobile") && a.name.includes("apk"))
        );
      } else if (p === "windows" || p === "win" || p === "exe" || p === "msi") {
        match = artifacts.find((a) => a.name === "avyro-windows-x64");
      } else if (p === "macos" || p === "mac" || p === "darwin" || p === "dmg") {
        match = artifacts.find((a) => a.name === "avyro-macos-arm64");
      } else if (p === "linux" || p === "deb" || p === "appimage") {
        match = artifacts.find((a) => a.name === "avyro-linux-x64");
      }
  ...
  try {
    ...
    if (release) {
      ...
      res.status(200).json({
        ...
        releaseName: release.name || "Avyro Protocol",
        publishedAt: release.published_at,
        downloads: {
          windows: {
            platform: "windows",
            name: "Windows (x64 Setup)",
            filename: winAsset?.name || "AVYRO_0.1.0_x64-setup.exe",
          ...
          macos: {
            ...
            filename: macAsset?.name || "AVYRO_aarch64.app.tar.gz",
          ...
          linux: {
            ...
            filename: linuxAsset?.name || "AVYRO_0.1.0_amd64.deb",
          ...
          android: {
            platform: "android",
            name: "Android (APK)",
            filename: apkAsset?.name || "avyro-mobile.apk",
            size: androidSize,
    ...
    res.status(200).json({
      version: "v0.1.0",
      releaseName: "Avyro Protocol",
      downloads: {
        windows: {
          platform: "windows",
          name: "Windows (x64 Setup)",
          filename: "AVYRO_0.1.0_x64-setup.exe",
          url: "/v1/downloads/windows",
        },
        macos: {
          platform: "macos",
          name: "macOS (Apple Silicon)",
          filename: "AVYRO_aarch64.dmg",
          url: "/v1/downloads/macos",
        },
        linux: {
          platform: "linux",
          name: "Linux (x64)",
          filename: "AVYRO_0.1.0_amd64.deb",
          url: "/v1/downloads/linux",
        },
        android: {
          platform: "android",
          name: "Android (APK)",
          filename: "avyro-mobile.apk",
          url: "/v1/downloads/android",
        },
      },
    });
...
downloadsRouter.get("/v1/downloads/mobile/latest", async (_req: Request, res: ExpressResponse): Promise<void> => {
  try {
    ...
    if (match) {
      ...
      res.status(200).json({
        version: release.tag_name || "v0.1.0",
        releaseName: release.name || "Avyro Protocol Mobile",
        publishedAt: release.published_at,
        android: {
          platform: "android",
          name: "Android (APK)",
    ...
    if (artMatch) {
      ...
      res.status(200).json({
        version: "v0.1.0",
        releaseName: "Avyro Protocol Mobile",
    ...
    res.status(200).json({
      version: "v0.1.0",
      releaseName: "Avyro Protocol Mobile for Android",
