...
paylinksRouter.post("/create", async (req: Request, res: Response) => {
  try {
    ...
    if (cleanToken.toLowerCase() === NATIVE_ETH.toLowerCase()) {
      symbol = "ETH";
    } else if (cleanToken.toLowerCase() === AVRO_TOKEN.toLowerCase()) {
      symbol = "AVRO";
    }
    ...
    try {
      ...
      const baseUrl = process.env.PUBLIC_APP_URL || "https://avroprotocol.com";
