...
import type { AvroWallet } from "@avro/protocol-sdk";
import { erc20Abi } from "@avro/protocol-sdk";
import { executeAccountCall } from "../lib/execute";
import { AVRO_TOKEN_ADDRESS, AVRO_TOKEN_CONFIGURED } from "../lib/tokens";
import {
...
export function StakingTab({
  ...
  const [avroBalance, setAvroBalance] = useState<string>("0.00");
  ...
  const refreshData = useCallback(async () => {
    ...
    try {
      ...
      // 3. Fetch on-chain $AVRO balance
      try {
        const bal = await client.readContract({
          address: AVRO_TOKEN_ADDRESS,
          abi: erc20Abi,
          functionName: "balanceOf",
          args: [wallet.address],
        });
        setAvroBalance(parseFloat(formatUnits(bal, 18)).toFixed(2));
      } catch (balErr) {
  ...
  const handleStake = async () => {
    if (!AVRO_TOKEN_CONFIGURED) {
      addToast("info", "$AVRO Not Live Yet", "Set VITE_AVRO_TOKEN_ADDRESS to the verified deployed contract before staking.");
      return;
    }
    const amountNum = parseFloat(stakeInput);
    if (isNaN(amountNum) || amountNum < 10000) {
      addToast("error", "Minimum 10,000 $AVRO Required", "Please enter at least 10,000 $AVRO to activate gasless transactions.");
      return;
    }
    ...
    try {
      ...
      addToast(
        "success",
        "Stake Successful",
        `Staked ${amountNum.toLocaleString()} $AVRO. You now have 100% gasless transactions sponsored by the protocol!`
      );
      setStakeInput("");
      await refreshData();
      onStakingUpdated?.();
  ...
  const handleRenew = async () => {
    if (!status?.stakedAmount || status.stakedAmount < 10000) {
      addToast("error", "Cannot Renew", "You need at least 10,000 $AVRO staked to renew your gasless pass.");
      return;
    }
  ...
  const handleUnstake = async () => {
    ...
    if (isNaN(amountNum) || amountNum <= 0) {
      addToast("error", "Invalid Amount", "Please enter a valid amount of $AVRO to unstake.");
      return;
    }

    if (amountNum > (status?.stakedAmount || 0)) {
      addToast("error", "Exceeds Balance", `You only have ${(status?.stakedAmount || 0).toLocaleString()} $AVRO staked.`);
      return;
    }
    ...
    try {
      ...
      addToast(
        ...
        `Unstaked ${amountNum.toLocaleString()} $AVRO. Tokens have been returned to your wallet.`
      );
      setUnstakeInput("");
      await refreshData();
      onStakingUpdated?.();
    ...
    return (
      <div className="flex-1 overflow-y-auto px-6 py-6 max-w-5xl mx-auto">
        <div className="rounded-2xl border border-white/10 bg-slate-950/70 p-8 text-center">
          ...
          <h2 className="text-2xl font-bold text-white">$AVRO staking is awaiting deployment</h2>
          <p className="mt-3 text-sm text-slate-400 max-w-xl mx-auto">
            The previous contract address has been removed. Configure the verified production contract with VITE_AVRO_TOKEN_ADDRESS before enabling staking.
          </p>
        </div>
      </div>
    );
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-white flex items-center gap-2">
            ...
            Stake $AVRO to Go Gasless
          </h2>
          <p className="text-sm text-slate-400 mt-1">
            Stake your $AVRO tokens each month to unlock 100% free transactions sponsored by the protocol.
          </p>
        </div>
        <button
          onClick={refreshData}
          ...
          <div className="space-y-2">
            ...
            <div className="text-3xl font-bold text-white tracking-tight">
              {isStaked ? (
                <span>
                  ...
                  <span className="text-lg font-normal text-slate-400">$AVRO Staked</span>
                </span>
              ) : (
            ...
            <p className="text-sm text-slate-300 max-w-xl">
              {isStaked
                ...
                : "Stake at least 10,000 $AVRO to stop paying network fees. You can unstake your tokens anytime."}
            </p>
          </div>
            ...
            {activeTab === "stake" ? (
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center text-xs text-slate-400">
                  ...
                  <span>
                    Available in Wallet: <b className="text-white">{avroBalance} $AVRO</b>
                  </span>
                </div>
                ...
                >
                  ...
                  {isStaking ? "Staking $AVRO..." : "Stake $AVRO & Activate Gasless"}
                </button>
              </div>
            ) : (
              /* Unstake Form */
              <div className="space-y-4">
                <div className="flex justify-between items-center text-xs text-slate-400">
                  ...
                  <span>
                    Currently Staked: <b className="text-white">{(status?.stakedAmount || 0).toLocaleString()} $AVRO</b>
                  </span>
                </div>
                ...
                <p className="text-xs text-slate-400 leading-relaxed">
                  When you unstake, your $AVRO tokens are transferred back to your wallet address. If your remaining staked balance falls below 10,000 $AVRO, gasless transaction sponsorship will pause.
                </p>
                ...
                >
                  <ArrowUpRight className="w-4 h-4 text-rose-400" />
                  {isUnstaking ? "Unstaking..." : "Unstake $AVRO to Wallet"}
                </button>
              </div>
            )}
          </div>
            ...
            <div className="space-y-2 text-xs">
              <div className="flex items-center justify-between p-2.5 rounded-lg bg-white/[0.03] border border-white/5">
                <div>
                  ...
                  <div className="text-[11px] text-slate-400">10,000 $AVRO</div>
                </div>
              ...
              <div className="flex items-center justify-between p-2.5 rounded-lg bg-white/[0.03] border border-white/5">
                <div>
                  ...
                  <div className="text-[11px] text-slate-400">50,000 $AVRO</div>
                </div>
              ...
              <div className="flex items-center justify-between p-2.5 rounded-lg bg-white/[0.03] border border-white/5">
                <div>
                  ...
                  <div className="text-[11px] text-slate-400">Every extra 50,000 $AVRO</div>
                </div>
              ...
              <div className="flex items-center justify-between p-2.5 rounded-lg bg-gradient-to-r from-amber-500/10 to-orange-500/10 border border-amber-500/20">
                <div>
                  ...
                  <div className="text-[11px] text-slate-400">1,000,000+ $AVRO</div>
                </div>
