...
export function PortfolioSparklineCard({
  ...
  useEffect(() => {
    ...
    async function loadChartData() {
      try {
        const cachedRaw = localStorage.getItem("avro_eth_24h_chart");
        const cachedTime = localStorage.getItem("avro_eth_24h_chart_time");
        const now = Date.now();
        ...
        if (Array.isArray(json?.prices) && json.prices.length > 0) {
          ...
          if (!cancelled) {
            ...
            try {
              localStorage.setItem("avro_eth_24h_chart", JSON.stringify(formatted));
              localStorage.setItem("avro_eth_24h_chart_time", String(now));
            } catch {}
          }
        }
      } catch {
