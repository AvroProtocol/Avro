...
export function generateStealthMetaAddressFromSeed(privateKeyHex: Hex): {
  ...
  const spendAvro = BigInt(privateKeyHex) % curveN;
  const spendPoint = secp256k1.Point.BASE.multiply(spendAvro);
  ...
  const viewPrivHash = keccak256(new TextEncoder().encode(`avro:viewing-key:${privateKeyHex}`));
...
export function computeStealthPrivateKey(
  ...
  const h = keccak256(bytesToHex(SxBytes));
  const hBig = BigInt(h) % curveN;
  const spendAvro = BigInt(spendAvroKeyHex) % curveN;
  const stealthPrivBig = (spendAvro + hBig) % curveN;
