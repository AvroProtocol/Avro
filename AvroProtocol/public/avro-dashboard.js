(() => {
  ...
  const initialActivity = [
    ...
    { id: 'PV-9031', type: 'Send', asset: 'USDC', amount: '-620.00', counterparty: 'ops.avro', time: 'Yesterday', status: 'Settled' },
    { id: 'PV-9027', type: 'Recovery', asset: 'Shard C', amount: 'Rotation', counterparty: 'Passkey verified', time: 'Yesterday', status: 'Settled' },
    { id: 'PV-9021', type: 'SDK', asset: 'Integration', amount: 'Copied', counterparty: '@avro/protocol-sdk', time: 'Yesterday', status: 'Settled' }
  ];
  ...
  function exportActivity() {
    ...
    link.href = url;
    link.download = 'avro-activity.csv';
    link.click();
  ...
  function downloadGuide() {
    const guide = [
      'AVRO PROTOCOL SDK Integration Guide',
      '',
      'Install:',
      ...
      '',
      'Status:',
      'SDK package target: @avro/protocol-sdk. Production co-signer: https://api.avroprotocol.com'
    ].join('\n');
    const blob = new Blob([guide], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = 'avro-integration-guide.txt';
    link.click();
    URL.revokeObjectURL(url);
    addActivity({ type: 'SDK', asset: 'Guide', amount: 'Downloaded', counterparty: 'avro-integration-guide.txt' });
  ...
  function bindEvents() {
    ...
    $('#copy-install')?.addEventListener('click', () => {
      copyText('npm install @avro/protocol-sdk viem', 'Install command copied', 'The SDK install command is on your clipboard.');
      addActivity({ type: 'SDK', asset: 'Install', amount: 'Copied', counterparty: '@avro/protocol-sdk' });
    });
