 ...
 *
 * Rebates are paid from AVRO the pool already holds rather than by market-buying
 * per claim: AVRO has roughly $4.8k of on-chain liquidity, where a $1.3k buy
 * already costs ~14% in slippage.
 *
 * The pool also custodies AVRO staked for gasless transactions. That stake is
...
/** Hard ceiling per claim, in AVRO wei. Bounds the blast radius of a mispriced claim. */
...
export interface PoolSolvency {
  address: string;
  chainId: number;
  /** AVRO the pool currently holds. */
  avroBalanceWei: string | null;
  /** AVRO owed back to stakers - untouchable by rebate payouts. */
  stakedReserveWei: string;
  /** AVRO promised to rebate claims not yet paid. */
...
/**
 * AVRO owed back to stakers, in wei.
 *
 * `staking_records.staked_amount` is a decimal AVRO quantity, so it is scaled to
...
export async function getStakedReserveWei(): Promise<{ wei: bigint; stakers: number }> {
  ...
  return { wei: avroDecimalToWei(decimal), stakers: result.rows[0].count };
}

/** Scales a decimal AVRO amount ("10000.5000") to wei without floating point. */
export function avroDecimalToWei(amount: string | number): bigint {
...
/** AVRO already promised to rebate claims but not yet paid. */
export async function getPendingRebateWei(): Promise<{ wei: bigint; claims: number }> {
  const result = await pool.query(
    `SELECT COALESCE(SUM(avro_wei), 0)::TEXT AS total, COUNT(*)::INT AS count
...
export async function getPoolSolvency(): Promise<PoolSolvency> {
  ...
  try {
    balanceWei = await getPoolAvroBalance();
  } catch (error) {
    console.error("[treasury] Pool AVRO balance read failed:", error);
  }
  ...
  return {
    ...
    avroBalanceWei: balanceWei?.toString() ?? null,
...
export async function getTreasuryStatus(): Promise<TreasuryStatus> {
  ...
  return {
    ...solvency,
    configured: solvency.avroBalanceWei !== null,
    payoutsEnabled: PAYOUTS_ENABLED,
    signerAvailable,
    avroToken: AVRO_TOKEN_ADDRESS,
  };
}
...
async function recordAttempt(
  ...
  await pool.query(
    `INSERT INTO rebate_payout_attempts (claim_id, treasury_address, avro_wei, tx_hash, outcome, error)
     VALUES ($1,$2,$3,$4,$5,$6)`,
    [claimId, POOL_ADDRESS, avroWei.toString(), txHash ?? null, outcome, error ?? null]
  );
}
...
export async function processPendingPayouts(
  ...
  try {
    ...
    throw new Error(`Could not read pool AVRO balance: ${(error as Error).message}`);
  }
  ...
  const candidates = await pool.query(
    `SELECT id, user_address, avro_wei FROM rebate_claims
  ...
  for (const claim of candidates.rows) {
    const avroWei = BigInt(claim.avro_wei);
    const recipient = claim.user_address as Address;

    if (!isAddress(recipient)) {
      await recordAttempt(claim.id, avroWei, "failed", undefined, "Invalid recipient");
      results.push({ claimId: claim.id, recipient, avroWei: claim.avro_wei, outcome: "failed", error: "Invalid recipient" });
      continue;
    }

    if (avroWei <= 0n || avroWei > MAX_PAYOUT_WEI) {
      const error = `Payout of ${avroWei} wei is outside the permitted range`;
      await recordAttempt(claim.id, avroWei, "failed", undefined, error);
      results.push({ claimId: claim.id, recipient, avroWei: claim.avro_wei, outcome: "failed", error });
      continue;
    }

    // Refuse rather than broadcast a transfer that would breach the stake
    // reserve or simply revert for want of balance.
    if (avroWei > spendable) {
      const error =
        `Pool has ${balanceWei} AVRO wei with ${staked.wei} reserved for stakers; ` +
        `${avroWei} exceeds the ${spendable > 0n ? spendable : 0n} available for rebates`;
      await recordAttempt(claim.id, avroWei, "failed", undefined, error);
      results.push({ claimId: claim.id, recipient, avroWei: claim.avro_wei, outcome: "skipped", error });
      continue;
    }

    if (dryRun) {
      await recordAttempt(claim.id, avroWei, "dry_run");
      results.push({ claimId: claim.id, recipient, avroWei: claim.avro_wei, outcome: "dry_run" });
      spendable -= avroWei;
      continue;
    }
    ...
    if (!locked) {
      results.push({ claimId: claim.id, recipient, avroWei: claim.avro_wei, outcome: "skipped" });
      continue;
    }
    ...
    try {
      const data = encodeFunctionData({
        ...
        args: [recipient, avroWei],
      });

      // Simulate first: a revert here costs nothing and leaves no half-state.
      await publicClient.call({ account: POOL_ADDRESS, to: AVRO_TOKEN_ADDRESS, data });

      const txHash = await signer!.client.sendTransaction({
        account: signer!.account,
        chain: null,
        to: AVRO_TOKEN_ADDRESS,
        data,
      });

      await recordAttempt(claim.id, avroWei, "sent", txHash);
      ...
      if (succeeded) spendable -= avroWei;

      results.push({
        claimId: claim.id,
        recipient,
        avroWei: claim.avro_wei,
      ...
      await recordAttempt(claim.id, avroWei, "failed", undefined, message);
      results.push({ claimId: claim.id, recipient, avroWei: claim.avro_wei, outcome: "failed", error: message });
    }
  }
