 ...
 * 3. Registers with live Co-Signer backend at api.avroprotocol.com
...
export async function createRealSmartAccount(): Promise<{
  ...
  try {
    ...
    await saveEncryptedItem(`avro_apikey_${realAddress}`, data.apiKey || "");
    await saveEncryptedItem(`avro_shard_b_${realAddress}`, data.shardBAddress || "");
    await saveEncryptedItem(`avro_shard_c_${realAddress}`, shardCKey);
    ...
    if (
      ...
      throw new Error(
        "Unable to connect to AVRO PROTOCOL security server. Please check your internet connection and try again."
      );
    }
    throw err;
  }
...
export async function loadStoredRecoveryKey(address: string): Promise<string | null> {
  ...
  try {
    return await loadEncryptedItem(`avro_shard_c_${address.toLowerCase()}`);
  } catch {
    return null;
  }
}
...
export async function loadStoredTransactions(address: string): Promise<LiveTransaction[]> {
  if (!address) return [];
  try {
    const raw = await loadEncryptedItem(`avro_txs_${address.toLowerCase()}`);
    if (!raw) return [];
...
export async function saveStoredTransactions(
  ...
  if (!address) return;
  try {
    await saveEncryptedItem(`avro_txs_${address.toLowerCase()}`, JSON.stringify(txs));
  } catch (err) {
...
export async function loadStoredPayLinks(address: string): Promise<MobilePayLink[]> {
  if (!address) return [];
  try {
    const raw = await loadEncryptedItem(`avro_paylinks_${address.toLowerCase()}`);
    if (!raw) return [];
    return JSON.parse(raw);
  } catch {
    return [];
...
export async function saveStoredPayLinks(
  ...
  if (!address) return;
  try {
    await saveEncryptedItem(`avro_paylinks_${address.toLowerCase()}`, JSON.stringify(links));
  } catch (err) {
...
export async function resetAllAppData(address?: string | null): Promise<void> {
  try {
    ...
    if (address) {
      ...
      await deleteEncryptedItem(`avro_shard_b_${cleanAddr}`);
      await deleteEncryptedItem(`avro_shard_c_${cleanAddr}`);
      await deleteEncryptedItem(`avro_apikey_${cleanAddr}`);
      await deleteEncryptedItem(`avro_txs_${cleanAddr}`);
      await deleteEncryptedItem(`avro_paylinks_${cleanAddr}`);
      await deleteEncryptedItem(`avro_guardrails_${cleanAddr}`);
      await deleteEncryptedItem(`avro_contacts_${cleanAddr}`);
      await deleteEncryptedItem(`avro_totp_${cleanAddr}`);
      await deleteEncryptedItem(`avro_totp_secret_${cleanAddr}`);
    }

    await deleteEncryptedItem("avro_contacts");
    await deleteEncryptedItem("avro_history");
    await deleteEncryptedItem("avro_paylinks");
    await deleteEncryptedItem("avro_guardrails");
    await deleteEncryptedItem("avro_totp");
    await deleteEncryptedItem("avro_totp_secret");
  } catch (err) {
