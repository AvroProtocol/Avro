...
/**
 * Mobile Spending Guardrails for AVRO PROTOCOL.
...
export async function loadMobileGuardrails(walletAddress: string): Promise<SpendingGuardrailConfig> {
  try {
    const raw = await loadEncryptedItem(`avro_guardrails_${walletAddress.toLowerCase()}`);
...
export async function saveMobileGuardrails(
  ...
  try {
    await saveEncryptedItem(
      `avro_guardrails_${walletAddress.toLowerCase()}`,
