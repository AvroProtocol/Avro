...
import { AVRO_API_URL } from "@/lib/api";
...
function FreezeComponent() {
  ...
  useEffect(() => {
    if (w && isAddressLike(w)) {
      try {
        localStorage.setItem("avro_freeze_wallet", w);
      } catch {
    ...
    try {
      const saved = localStorage.getItem("avro_freeze_wallet");
