...
import { useAvroConfig } from "@/lib/avro-config";
import { usePageReveal } from "@/lib/use-page-reveal";
import { avroApiUrl } from "@/lib/api";
import "@/styles/avro-site.css";

export const Route = createFileRoute("/")({
  component: AvroLandingPage,
  head: () => ({
    meta: [
      { title: "Avro Protocol | Private payments. Real ownership." },
      {
        name: "description",
...
const DOWNLOADS = [
  ["Windows", "Windows 10 / 11 · x64", avroApiUrl("v1/downloads/windows")],
  ["macOS", "macOS 11+ · Apple Silicon", avroApiUrl("v1/downloads/macos")],
  ["Linux", "Debian / AppImage", avroApiUrl("v1/downloads/linux")],
] as const;
...
const FAQS = [
  ["What is Avro?", "Avro is a self-custodial smart wallet and open developer platform for private payments. It combines ERC-4337 smart accounts, threshold key security, passkey recovery, payment links, swaps, and privacy infrastructure."],
  ...
  ["Can Avro move funds by itself?", "No. The policy co-signer cannot spend alone. The account contract requires a valid threshold quorum before execution."],
  ...
  ["Which network does Avro support?", "Avro is engineered for Robinhood Chain, an Arbitrum-powered EVM L2 with Chain ID 4663. Native ETH is used for gas and USDG is a core settlement asset."],
] as const;
    ...
    <div className="nx-quorum">
      <div className="nx-quorum-radar" aria-hidden="true">
        ...
        <div className={`nx-quorum-core ${ready ? "is-ready" : ""}`}>
          <img src="/assets/avro-mark-white.png" alt="" />
          <b>{count}/3</b>
...
function AvroLandingPage() {
  usePageReveal();
  const { config, buyUrl } = useAvroConfig();
    ...
    <div className="av-site nx-site">
      ...
      <main>
        <section className="nx-hero" id="overview" ref={heroRef}>
          ...
          <div className="nx-hero-ghost" aria-hidden="true">AVRO</div>
          ...
          <div className="nx-container nx-hero-layout">
            <div className="nx-hero-copy" data-reveal>
              ...
              <p className="nx-hero-lead">Avro turns smart-account security into a product you can actually use. Pay, receive, swap, recover, and build without handing one key or one company total control.</p>
              <div className="nx-hero-actions">
                <a href="#product" className="nx-button nx-button-primary">Enter the protocol <ArrowRight size={16} /></a>
                <a href="/avro" className="nx-button nx-button-ghost">${config.ticker} <ArrowUpRight size={15} /></a>
              </div>
          ...
          <a className="nx-scroll-cue" href="#download"><span>DOWNLOAD AVRO</span><ArrowDown size={14} /></a>
        ...
        <section className="nx-quick-downloads" id="download">
          <div className="nx-container nx-quick-download-panel" data-reveal>
            <div className="nx-quick-download-copy">
              <span className="nx-index">GET AVRO</span>
              <h2>Choose your client.</h2>
              <p>Install the native AVRO client directly from the official release channel.</p>
            </div>
        ...
        <section className="nx-section nx-security" id="security">
          ...
          <div className="nx-container nx-security-layout">
            <div className="nx-security-copy" data-reveal>
              ...
              <p>Avro makes the security boundary visible. A remote service cannot spend alone. A stolen device cannot spend alone. Recovery cannot spend alone.</p>
        ...
        <section className="nx-type-break" aria-hidden="true"><div>AVRO <span>AVRO</span> AVRO <span>AVRO</span></div></section>
        ...
        <section className="nx-section nx-network" id="network">
          <div className="nx-container">
            <div className="nx-network-panel" data-reveal>
              ...
              <div className="nx-network-map" aria-hidden="true"><div className="nx-net-center"><img src="/assets/avro-mark-white.png" alt=""/></div><i className="nx-net-ring nx-net-r1"/><i className="nx-net-ring nx-net-r2"/><span className="nx-net-a">4337</span><span className="nx-net-b">ETH</span><span className="nx-net-c">USDG</span><span className="nx-net-d">4663</span></div>
            </div>
          </div>
        </section>
        ...
        <section className="nx-section nx-mobile-home" id="mobile">
          <div className="nx-container nx-mobile-home-layout">
            <div className="nx-mobile-home-copy" data-reveal>
              ...
              <p>The mobile experience is now part of the core Avro story. Send, receive, manage limits, create pay links, and recover access without shrinking a desktop dashboard into a phone.</p>
            ...
            <div className="nx-mobile-device-stage" data-reveal aria-label="Avro mobile wallet preview">
              ...
              <div className="nx-mobile-phone">
                ...
                <div className="nx-mobile-screen">
                  <div className="nx-mobile-top"><img src="/assets/avro-mark-white.png" alt=""/><span>AVRO</span><i/></div>
        ...
        <section className="nx-section nx-token" id="token">
          <div className="nx-container nx-token-panel" data-reveal>
            <div className="nx-token-title"><span className="nx-index nx-index-dark">08 / ECOSYSTEM</span><h2><small>$</small>{config.ticker}</h2><p>Official launch access and the verified contract address will appear here when $AVRO goes live.</p></div>
            <div className="nx-token-live">
              ...
              <div className="nx-token-actions"><a href="/avro" className="nx-button nx-button-light">Token page <ArrowRight size={15}/></a>{buyUrl ? <a href={buyUrl} target="_blank" rel="noreferrer" className="nx-button nx-button-outline-dark">Buy ${config.ticker} <ArrowUpRight size={15}/></a> : <span className="nx-button nx-button-outline-dark is-disabled">Buy coming soon</span>}</div>
            </div>
          </div>
        </section>
        ...
        <section className="nx-final">
          ...
          <div className="nx-container" data-reveal>
            <div className="nx-final-mark"><img src="/assets/avro-mark-white.png" alt=""/><span><Sparkles size={14}/> AVRO PROTOCOL</span></div>
            <h2>Keep the keys.<br/>Move the value.</h2>
            <p>Private payments and programmable self-custody for a more open financial world.</p>
            <div className="nx-hero-actions"><a href="#product" className="nx-button nx-button-light">Explore Avro <ArrowRight size={15}/></a>{config.xUrl && <a href={config.xUrl} target="_blank" rel="noreferrer" className="nx-button nx-button-outline-dark">Follow on X <ArrowUpRight size={15}/></a>}</div>
          </div>
        </section>
      </main>
      <AvroFooter />
