...
export interface AvroWalletRegistration {
  address: Address;
  chainId: number;
...
export interface AvroWalletConfig {
  address: Address;
  shardA: ILocalShard;
...
/* ---------- Cross-chain bridging & AVRO rebates ---------- */

/** Chains Avro bridges between (Ethereum, Base, Arbitrum, Robinhood Chain). */
...
export interface BridgeQuoteRequest {
  ...
  /** Wallet that accrues Avro's app fee with Relay. */
  appFeeRecipient?: Address;
  /** Avro's app fee in basis points. */
  appFeeBps?: number;
}
...
export interface BridgeRebateEstimate {
  ...
  rebateUsd: string;
  rebateBps: number;
  /** Rebates are always denominated for payout in AVRO. */
  rebateCurrency: string;
  /** AVRO only exists on Robinhood Chain, so rebates settle there. */
