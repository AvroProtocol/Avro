...
Viem-native TypeScript SDK enabling developers to embed AVRO PROTOCOL 2-of-3 threshold self-custody directly into applications on Robinhood Chain (Chain ID: 4663).
...
- **Gasless Staking & Cross-Chain Bridging**: Protocol staking tier calculations and cross-chain bridge quote with AVRO relayer rebate math.

## Installation

```bash
bun add @avro/protocol-sdk viem
# or
npm install @avro/protocol-sdk viem
```

## Quick Start: Create Wallet

Generate a device key locally and register a new threshold smart account with the live AVRO PROTOCOL co-signer:

```typescript
import { AvroWallet, LocalShard } from "@avro/protocol-sdk";

// 1. Generate local device key (Shard A)
const shardA = LocalShard.create("device");

// 2. Register wallet with the live AVRO PROTOCOL co-signer
const { wallet, shardC } = await AvroWallet.create({
...
const receipt = await AvroWallet.recoverWallet({
...
console.log("PayLink URL:", `https://avroprotocol.com/pay/${paylink.slug}`);
...
Calculate monthly transaction quotas based on staked $AVRO:
...
Estimate cross-chain bridging routes via Relay.link with automated $AVRO rebates on the relayer margin:
...
console.log("Rebate USD in AVRO:", quote.rebate.rebateUsd);
```
...
console.log("Factory:", AVRO_FACTORY_ADDRESS);
```
