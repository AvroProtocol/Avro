(() => {
  ...
  const initialActivity = [
    ...
    { id: 'PV-9031', type: 'Send', asset: 'USDC', amount: '-620.00', counterparty: 'ops.avyro', time: 'Yesterday', status: 'Settled' },
    { id: 'PV-9027', type: 'Recovery', asset: 'Shard C', amount: 'Rotation', counterparty: 'Passkey verified', time: 'Yesterday', status: 'Settled' },
    { id: 'PV-9021', type: 'SDK', asset: 'Integration', amount: 'Copied', counterparty: '@avyro/protocol-sdk', time: 'Yesterday', status: 'Settled' }
  ];
  ...
  function exportActivity() {
    ...
    link.href = url;
    link.download = 'avyro-activity.csv';
    link.click();
  ...
  function downloadGuide() {
    const guide = [
      'AVYRO PROTOCOL SDK Integration Guide',
      '',
      'Install:',
      'npm install @avyro/protocol-sdk viem',
      '',
      'Network:',
      ...
      '',
      'Status:',
      'SDK package target: @avyro/protocol-sdk. Production co-signer: https://api.avyroprotocol.com'
    ].join('\n');
    const blob = new Blob([guide], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = 'avyro-integration-guide.txt';
    link.click();
    URL.revokeObjectURL(url);
    addActivity({ type: 'SDK', asset: 'Guide', amount: 'Downloaded', counterparty: 'avyro-integration-guide.txt' });
  ...
  function bindEvents() {
    ...
    $('#copy-install')?.addEventListener('click', () => {
      copyText('npm install @avyro/protocol-sdk viem', 'Install command copied', 'The SDK install command is on your clipboard.');
      addActivity({ type: 'SDK', asset: 'Install', amount: 'Copied', counterparty: '@avyro/protocol-sdk' });
    });
