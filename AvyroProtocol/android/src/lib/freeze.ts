...
export async function freezeMobileWallet(
  ...
  const cleanAddr = walletAddress.trim().toLowerCase();
  const apiKey = await loadEncryptedItem(`avyro_apikey_${cleanAddr}`);
...
export async function setupMobileTotp(
  ...
  const data = await res.json();
  await saveEncryptedItem(`avyro_totp_pending_${cleanAddr}`, data.secret);
  return {
    secret: data.secret,
    uri: data.uri,
  };
...
export async function confirmMobileTotp(
  ...
  const data = await res.json();
  if (data.ok) {
    await saveEncryptedItem(`avyro_totp_enabled_${cleanAddr}`, "true");
    return true;
  }
  return false;
}
