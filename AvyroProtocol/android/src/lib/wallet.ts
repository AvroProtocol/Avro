 ...
 * 3. Registers with live Co-Signer backend at api.avyroprotocol.com
...
export async function createRealSmartAccount(): Promise<{
  ...
  try {
    ...
    await saveEncryptedItem(`avyro_apikey_${realAddress}`, data.apiKey || "");
    await saveEncryptedItem(`avyro_shard_b_${realAddress}`, data.shardBAddress || "");
    await saveEncryptedItem(`avyro_shard_c_${realAddress}`, shardCKey);
    ...
    if (
      ...
      throw new Error(
        "Unable to connect to AVYRO PROTOCOL security server. Please check your internet connection and try again."
      );
    }
    throw err;
  }
...
export async function loadStoredRecoveryKey(address: string): Promise<string | null> {
  ...
  try {
    return await loadEncryptedItem(`avyro_shard_c_${address.toLowerCase()}`);
  } catch {
    return null;
  }
}
...
export async function loadStoredTransactions(address: string): Promise<LiveTransaction[]> {
  if (!address) return [];
  try {
    const raw = await loadEncryptedItem(`avyro_txs_${address.toLowerCase()}`);
    if (!raw) return [];
...
export async function saveStoredTransactions(
  ...
  if (!address) return;
  try {
    await saveEncryptedItem(`avyro_txs_${address.toLowerCase()}`, JSON.stringify(txs));
  } catch (err) {
...
export async function loadStoredPayLinks(address: string): Promise<MobilePayLink[]> {
  if (!address) return [];
  try {
    const raw = await loadEncryptedItem(`avyro_paylinks_${address.toLowerCase()}`);
    if (!raw) return [];
    return JSON.parse(raw);
  } catch {
    return [];
...
export async function saveStoredPayLinks(
  ...
  if (!address) return;
  try {
    await saveEncryptedItem(`avyro_paylinks_${address.toLowerCase()}`, JSON.stringify(links));
  } catch (err) {
...
export async function resetAllAppData(address?: string | null): Promise<void> {
  try {
    ...
    if (address) {
      ...
      await deleteEncryptedItem(`avyro_shard_b_${cleanAddr}`);
      await deleteEncryptedItem(`avyro_shard_c_${cleanAddr}`);
      await deleteEncryptedItem(`avyro_apikey_${cleanAddr}`);
      await deleteEncryptedItem(`avyro_txs_${cleanAddr}`);
      await deleteEncryptedItem(`avyro_paylinks_${cleanAddr}`);
      await deleteEncryptedItem(`avyro_guardrails_${cleanAddr}`);
      await deleteEncryptedItem(`avyro_contacts_${cleanAddr}`);
      await deleteEncryptedItem(`avyro_totp_${cleanAddr}`);
      await deleteEncryptedItem(`avyro_totp_secret_${cleanAddr}`);
    }

    await deleteEncryptedItem("avyro_contacts");
    await deleteEncryptedItem("avyro_history");
    await deleteEncryptedItem("avyro_paylinks");
    await deleteEncryptedItem("avyro_guardrails");
    await deleteEncryptedItem("avyro_totp");
    await deleteEncryptedItem("avyro_totp_secret");
  } catch (err) {
