...
/**
 * Mobile Spending Guardrails for AVYRO PROTOCOL.
...
export async function loadMobileGuardrails(walletAddress: string): Promise<SpendingGuardrailConfig> {
  try {
    const raw = await loadEncryptedItem(`avyro_guardrails_${walletAddress.toLowerCase()}`);
...
export async function saveMobileGuardrails(
  ...
  try {
    await saveEncryptedItem(
      `avyro_guardrails_${walletAddress.toLowerCase()}`,
