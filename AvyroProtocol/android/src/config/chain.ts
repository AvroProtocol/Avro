export const ROBINHOOD_CHAIN_ID = 4663;
export const ROBINHOOD_CHAIN_NAME = "Robinhood Chain";
export const ROBINHOOD_RPC_URL = "https://rpc.mainnet.chain.robinhood.com";
export const ROBINHOOD_EXPLORER_URL = "https://robinhoodchain.blockscout.com";

export const ENTRY_POINT_ADDRESS = "0x5FF137D4b0FDCD49DcA30c7CF57E578a026d2789";
export const AVYRO_FACTORY_ADDRESS = "0x5308e6f5faafa7ef6fff948994bd396a982be0d0";
export const USDG_TOKEN_ADDRESS = "0x5fc5360D0400a0Fd4f2af552ADD042D716F1d168";

export const COSIGNER_API_URL = (process.env.EXPO_PUBLIC_AVYRO_API_URL || "https://api.avyroprotocol.com").replace(/\/$/, "");
export const PUBLIC_APP_URL = "https://avyroprotocol.com";
export const NATIVE_ETH_ADDRESS = "0x0000000000000000000000000000000000000000";
