...
export function generateStealthMetaAddressFromSeed(privateKeyHex: Hex): {
  ...
  const spendAvyr = BigInt(privateKeyHex) % curveN;
  const spendPoint = secp256k1.Point.BASE.multiply(spendAvyr);
  ...
  const viewPrivHash = keccak256(new TextEncoder().encode(`avyro:viewing-key:${privateKeyHex}`));
...
export function computeStealthPrivateKey(
  ...
  const h = keccak256(bytesToHex(SxBytes));
  const hBig = BigInt(h) % curveN;
  const spendAvyr = BigInt(spendAvyrKeyHex) % curveN;
  const stealthPrivBig = (spendAvyr + hBig) % curveN;
