...
export function App() {
  const [desktopTheme, setDesktopTheme] = useState<"dark" | "light">(() => {
    try {
      return localStorage.getItem("avyro.desktop.theme") === "light" ? "light" : "dark";
    } catch {
      return "dark";
    }
  });
  ...
  const toggleDesktopTheme = useCallback(() => {
    setDesktopTheme((current) => {
      ...
      try { localStorage.setItem("avyro.desktop.theme", next); } catch {}
      return next;
    });
  }, []);
  ...
  const [accounts, setAccounts] = useState<WalletAccount[]>(() => {
    const activeWalletAddr = typeof window !== "undefined" ? localStorage.getItem("avyro_wallet_address") : null;
    const initialAddr = (activeWalletAddr as Address) || ("0x0000000000000000000000000000000000000000" as Address);
    try {
      const stored = localStorage.getItem("avyro_accounts");
      if (stored) {
  ...
  const [wallet, setWallet] = useState<AvyroWallet | null>(null);
  ...
  useEffect(() => {
    if (walletAddress && walletAddress !== "0x0000000000000000000000000000000000000000") {
      setAccounts((prev) => {
        ...
        if (changed) {
          try {
            localStorage.setItem("avyro_accounts", JSON.stringify(updated));
          } catch {}
          return updated;
        }
        return prev;
  ...
  const [ethPrice, setEthPrice] = useState<number>(() => {
    try {
      const cached = localStorage.getItem("avyro_eth_price");
  ...
  const [eth24hChange, setEth24hChange] = useState<number>(() => {
    try {
      const cached = localStorage.getItem("avyro_eth_24h_change");
  ...
  const [lastPriceFetchTime, setLastPriceFetchTime] = useState<number>(() => {
    try {
      const cached = localStorage.getItem("avyro_eth_price_time");
  ...
  const [totpVerified, setTotpVerified] = useState<boolean>(() => {
    try {
      const activeId = localStorage.getItem("avyro_active_account_id") || "primary";
      const activeAddr = localStorage.getItem(`avyro_wallet_address_${activeId}`) || localStorage.getItem("avyro_wallet_address");
      if (activeAddr && localStorage.getItem(`avyro_totp_enrolled_${activeAddr.toLowerCase()}`) === "true") {
        return true;
      }
      if (localStorage.getItem(`avyro_totp_enrolled_${activeId}`) === "true") {
        return true;
      }
      if (activeId === "primary" && localStorage.getItem("avyro_totp_enrolled") === "true") {
        return true;
      }
      return false;
    } catch {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (typeof p === "number" && p > 0) {
          ...
          try {
            localStorage.setItem("avyro_eth_price", String(p));
            localStorage.setItem("avyro_eth_price_time", String(now));
          } catch {}
        }
        if (typeof change === "number") {
          setEth24hChange(change);
          try {
            localStorage.setItem("avyro_eth_24h_change", String(change));
          } catch {}
        }
        return p;
      }
  ...
  useEffect(() => {
    ...
    try {
      const key = `avyro_transactions_${walletAddress.toLowerCase()}`;
  ...
  useEffect(() => {
    ...
    async function loadSavedWallet() {
      try {
        ...
        let savedAddress = localStorage.getItem(`avyro_wallet_address_${activeId}`) || (activeId === "primary" ? localStorage.getItem("avyro_wallet_address") : null);

        let savedShardA: string | null = null;
        if (savedAddress) {
          savedShardA = localStorage.getItem(`avyro_shard_a_${savedAddress.toLowerCase()}`);
        }
        if (!savedShardA) {
          savedShardA = localStorage.getItem(`avyro_shard_a_${activeId}`) || (activeId === "primary" ? localStorage.getItem("avyro_shard_a") : null);
        }
        ...
        if (savedShardA) {
          try {
            ...
            if (savedAddress && derived.toLowerCase() !== savedAddress.toLowerCase()) {
              console.warn(`[Avyro Protocol] Key alignment: loaded shard A derives ${derived} while saved address was ${savedAddress}. Synchronizing active wallet address.`);
              savedAddress = derived;
            }
          } catch {}
        }

        const savedApiKey = localStorage.getItem(`avyro_api_key_${activeId}`) || (activeId === "primary" ? localStorage.getItem("avyro_api_key") : null);
        const savedShardB = localStorage.getItem(`avyro_shard_b_address_${activeId}`) || (activeId === "primary" ? localStorage.getItem("avyro_shard_b_address") : null);
        const savedShardC = localStorage.getItem(`avyro_shard_c_address_${activeId}`) || (activeId === "primary" ? localStorage.getItem("avyro_shard_c_address") : null);
        ...
        if (savedShardA && savedAddress && savedApiKey && savedShardB && savedShardC) {
          ...
          const restoredWallet = new AvyroWallet({
          ...
          localStorage.removeItem("avyro_shard_c_key");

          const isTotp =
            localStorage.getItem(`avyro_totp_enrolled_${savedAddress.toLowerCase()}`) === "true" ||
            localStorage.getItem(`avyro_totp_enrolled_${activeId}`) === "true" ||
            (activeId === "primary" && localStorage.getItem("avyro_totp_enrolled") === "true");
  ...
  const handleSelectAccount = async (targetId: string) => {
    ...
    try {
      localStorage.setItem("avyro_active_account_id", targetId);
    } catch {}

    const savedAddress = localStorage.getItem(`avyro_wallet_address_${targetId}`) || (targetId === "primary" ? localStorage.getItem("avyro_wallet_address") : null);
    ...
    if (!savedShardA) {
      savedShardA = localStorage.getItem(`avyro_shard_a_${targetId}`) || (targetId === "primary" ? localStorage.getItem("avyro_shard_a") : null);
    }
    if (!savedShardA && isTauri()) {
      if (savedAddress) {
        try {
    ...
    const savedApiKey = localStorage.getItem(`avyro_api_key_${targetId}`) || (targetId === "primary" ? localStorage.getItem("avyro_api_key") : null);
    const savedShardB = localStorage.getItem(`avyro_shard_b_address_${targetId}`) || (targetId === "primary" ? localStorage.getItem("avyro_shard_b_address") : null);
    const savedShardC = localStorage.getItem(`avyro_shard_c_address_${targetId}`) || (targetId === "primary" ? localStorage.getItem("avyro_shard_c_address") : null);
    ...
    if (savedShardA && savedAddress && savedApiKey && savedShardB && savedShardC) {
      ...
      const isTotp =
        localStorage.getItem(`avyro_totp_enrolled_${savedAddress.toLowerCase()}`) === "true" ||
        localStorage.getItem(`avyro_totp_enrolled_${targetId}`) === "true" ||
        (targetId === "primary" && localStorage.getItem("avyro_totp_enrolled") === "true");
      setTotpVerified(isTotp);
      setTotpSecret(null);
      setTotpQrCode(null);
      setTotpCode("");
  ...
  const handleConfirmRemoveAccount = async (target: WalletAccount) => {
    try {
      ...
      localStorage.removeItem(`avyro_wallet_address_${target.id}`);
      localStorage.removeItem(`avyro_shard_a_${target.id}`);
      if (cleanAddress) {
        localStorage.removeItem(`avyro_shard_a_${cleanAddress}`);
        localStorage.removeItem(`avyro_totp_enrolled_${cleanAddress}`);
        localStorage.removeItem(`avyro_transactions_${cleanAddress}`);
      }
      localStorage.removeItem(`avyro_shard_b_address_${target.id}`);
      localStorage.removeItem(`avyro_shard_c_address_${target.id}`);
      localStorage.removeItem(`avyro_api_key_${target.id}`);
      localStorage.removeItem(`avyro_totp_enrolled_${target.id}`);

      if (target.id === "primary") {
        localStorage.removeItem("avyro_wallet_address");
        localStorage.removeItem("avyro_shard_a");
        localStorage.removeItem("avyro_shard_b_address");
        localStorage.removeItem("avyro_shard_c_address");
        localStorage.removeItem("avyro_api_key");
        localStorage.removeItem("avyro_totp_enrolled");
      }
      ...
      if (remaining.length === 0) {
        ...
        setTransactions([]);
        try {
          localStorage.removeItem("avyro_accounts");
          localStorage.removeItem("avyro_active_account_id");
        } catch {}
        addToast("info", "Wallet Removed", `${displayName} was removed. No wallets remaining on this device.`);
      } else {
        setAccounts(remaining);
        try {
          localStorage.setItem("avyro_accounts", JSON.stringify(remaining));
        } catch {}
  ...
  const handleCreateWallet = async () => {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        const { wallet: newWallet, shardC } = await AvyroWallet.create();
        ...
        localStorage.setItem(`avyro_shard_a_${newAccId}`, newWallet.shardA.privateKey);
        localStorage.setItem(`avyro_shard_a_${newWallet.address.toLowerCase()}`, newWallet.shardA.privateKey);
        if (newAccId === "primary") {
          localStorage.setItem("avyro_shard_a", newWallet.shardA.privateKey);
        }

        // Save account-specific credentials
        localStorage.setItem(`avyro_wallet_address_${newAccId}`, newWallet.address);
        localStorage.setItem(`avyro_api_key_${newAccId}`, newWallet.apiKey);
        localStorage.setItem(`avyro_shard_b_address_${newAccId}`, newWallet.shardB.address);
        localStorage.setItem(`avyro_shard_c_address_${newAccId}`, shardC.address);

        if (newAccId === "primary") {
          localStorage.setItem("avyro_wallet_address", newWallet.address);
          localStorage.setItem("avyro_api_key", newWallet.apiKey);
          localStorage.setItem("avyro_shard_b_address", newWallet.shardB.address);
          localStorage.setItem("avyro_shard_c_address", shardC.address);
        }
        ...
        try {
          localStorage.setItem("avyro_accounts", JSON.stringify(updatedAccounts));
          localStorage.setItem("avyro_active_account_id", newAccId);
        } catch {}
        ...
        try {
          localStorage.setItem(`avyro_totp_enrolled_${newAccId}`, "false");
          localStorage.setItem(`avyro_totp_enrolled_${newWallet.address.toLowerCase()}`, "false");
          if (newAccId === "primary") {
            localStorage.setItem("avyro_totp_enrolled", "false");
          }
          localStorage.setItem(`avyro_transactions_${newWallet.address.toLowerCase()}`, JSON.stringify([]));
        } catch {}
  ...
  const handleImportPrivateKey = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem(`avyro_shard_a_${accId}`, cleanKey);
      localStorage.setItem(`avyro_shard_a_${cleanAddress}`, cleanKey);
      localStorage.setItem(`avyro_wallet_address_${accId}`, cleanAddress);
      if (accId === "primary") {
        localStorage.setItem("avyro_shard_a", cleanKey);
        localStorage.setItem("avyro_wallet_address", cleanAddress);
      }
      ...
      try {
        ...
        if (infoRes.ok) {
          ...
          localStorage.setItem(`avyro_shard_b_address_${accId}`, shardBAddress);
          localStorage.setItem(`avyro_shard_c_address_${accId}`, shardCAddr);
          localStorage.setItem(`avyro_api_key_${accId}`, apiKeyVal);
        }
      } catch {}

      const shardB = new RemoteCosigner(shardBAddress, cleanAddress, DEFAULT_API_URL);
      const importedWallet = new AvyroWallet({
      ...
      setAccounts((prev) => {
        ...
        try {
          localStorage.setItem("avyro_accounts", JSON.stringify(updated));
          localStorage.setItem("avyro_active_account_id", accId);
        } catch {}
        return updated;
      });
  ...
  const handleRecoverWallet = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        await AvyroWallet.recoverWallet({
      ...
      localStorage.setItem(`avyro_shard_a_${accId}`, newShardA.privateKey);
      localStorage.setItem(`avyro_shard_a_${cleanAddress}`, newShardA.privateKey);
      localStorage.setItem(`avyro_wallet_address_${accId}`, cleanAddress);
      localStorage.setItem(`avyro_shard_c_address_${accId}`, shardC.address);
      localStorage.setItem("avyro_totp_enrolled", "true");
      if (accId === "primary") {
        localStorage.setItem("avyro_shard_a", newShardA.privateKey);
        localStorage.setItem("avyro_wallet_address", cleanAddress);
        localStorage.setItem("avyro_shard_c_address", shardC.address);
      }

      // 4. Fetch wallet info from cosigner to construct AvyroWallet
      let shardBAddress: Address = "0x0000000000000000000000000000000000000000";
      let apiKeyVal = "";
      try {
        const infoRes = await fetch(`${DEFAULT_API_URL}/v1/wallets/${cleanAddress}`);
        if (infoRes.ok) {
          ...
          localStorage.setItem(`avyro_api_key_${accId}`, apiKeyVal);
          if (accId === "primary") {
            localStorage.setItem("avyro_shard_b_address", shardBAddress);
            localStorage.setItem("avyro_api_key", apiKeyVal);
          }
        }
      } catch {}
  ...
  const handleSendTransaction = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (isStealthSend && isMeta) {
        ...
        if (wallet?.address) {
          try {
            localStorage.setItem(`avyro_transactions_${wallet.address.toLowerCase()}`, JSON.stringify(updatedList));
          } catch {}
      ...
      const signature = await timeStage("combine", onStage, async () =>
        AvyroWallet.combineSignatures(sigA, sigB)
      );
  ...
  const handleConfirmTotp = async () => {
    ...
    try {
      ...
      if (success) {
        setTotpVerified(true);
        try {
          localStorage.setItem(`avyro_totp_enrolled_${activeAccountId}`, "true");
          if (wallet.address) {
            localStorage.setItem(`avyro_totp_enrolled_${wallet.address.toLowerCase()}`, "true");
          }
  ...
  return (
    <div className={`min-h-screen h-screen flex flex-col bg-[#050505] text-slate-100 font-sans overflow-hidden selection:bg-white/20 ${desktopTheme === "light" ? "avyro-desktop-light" : "avyro-desktop-dark"}`}>
      {/* 1.5s Splash Screen with Avyro Protocol Logo */}
      {showSplash && (
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6 text-center max-w-xl mx-auto w-full space-y-7 animate-in fade-in duration-500 overflow-y-auto select-none relative">
          ...
          <div className="flex flex-col items-center space-y-4">
            <div className="w-20 h-20 rounded-2xl bg-white/[0.04] border border-white/10 p-3.5 flex items-center justify-center shadow-2xl">
              <img src="/logo.png" alt="Avyro Protocol" className="w-full h-full object-contain" />
            </div>
            <div>
              <h1 className="text-3xl font-semibold tracking-tight text-white">Avyro Protocol Self-Custody</h1>
          ...
          <div className="w-full space-y-2.5 pt-2 text-left">
            ...
            <div className="rounded-xl bg-[#181a22] border border-white/[0.08] overflow-hidden">
              ...
              >
                <div className="text-xs font-semibold text-white flex items-center gap-2">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#3f3f46]"></span>
                  <span>How is Avyro Protocol different from regular wallets?</span>
                </div>
                <ChevronDown
                  className={`w-3.5 h-3.5 text-slate-400 transition-transform duration-200 shrink-0 ${
              ...
              {openFaqIndex === 1 && (
                <div className="px-4 pb-4 text-xs text-slate-400 leading-relaxed pl-7.5 border-t border-white/5 pt-2.5">
                  Regular wallets rely on a single, vulnerable 12-word seed phrase that presents a single point of failure. Avyro Protocol splits protection into a 2-of-3 threshold quorum: your local device key (Shard A), an automated co-signer (Shard B), and an emergency backup (Shard C) protected by 2FA. No single key can ever steal your assets.
                </div>
              )}
            </div>
          </div>
          ...
          <div className="pt-2 flex items-center justify-center gap-6 text-xs text-slate-500">
            <a
              href="https://avyroprotocol.com"
              target="_blank"
              rel="noopener noreferrer"
              className="hover:text-slate-300 transition flex items-center gap-1.5"
            >
              <span>Avyro Protocol Web</span>
              <ExternalLink className="w-3 h-3" />
            </a>
            <span className="text-slate-700">•</span>
            <a
              href="https://x.com/AvyroProtocol"
              target="_blank"
              rel="noopener noreferrer"
              className="hover:text-slate-300 transition flex items-center gap-1.5"
            >
              <span>Avyro Protocol on X</span>
              <ExternalLink className="w-3 h-3" />
            </a>
          </div>
        </div>
            ...
            <div className="p-4 rounded-xl bg-[#181a22] border border-white/[0.08] flex items-center justify-between">
              <div className="flex items-center gap-3.5">
                ...
                <div>
                  ...
                  <div className="text-xs text-slate-400 mt-0.5 font-mono">
                    {wallet ? `Co-signer: ${shortenAddress(wallet.shardB.address)} (api.avyroprotocol.com)` : "api.avyroprotocol.com"}
                  </div>
                </div>
              </div>
              <span className="text-xs font-medium px-2.5 py-1 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 flex items-center gap-1.5">
      ...
      {/* Avyro Protocol Assistant Widget (V2 On-Device AI) */}
      <AssistantWidget
