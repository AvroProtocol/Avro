...
export function TransactionReceiptCard({
  ...
  const saveImage = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `avyro-receipt-${receipt.hash.slice(2, 12)}.png`;
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        ...
        <button
          type="button"
          onClick={onDone}
          className="flex-1 py-2.5 rounded-xl bg-[#3f3f46] hover:bg-[#52525b] text-white font-semibold text-xs transition"
        >
          Done
        </button>
      </div>
