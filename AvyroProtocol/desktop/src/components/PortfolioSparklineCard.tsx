...
export function PortfolioSparklineCard({
  ...
  useEffect(() => {
    ...
    async function loadChartData() {
      try {
        const cachedRaw = localStorage.getItem("avyro_eth_24h_chart");
        const cachedTime = localStorage.getItem("avyro_eth_24h_chart_time");
        const now = Date.now();
        ...
        if (Array.isArray(json?.prices) && json.prices.length > 0) {
          ...
          if (!cancelled) {
            ...
            try {
              localStorage.setItem("avyro_eth_24h_chart", JSON.stringify(formatted));
              localStorage.setItem("avyro_eth_24h_chart_time", String(now));
            } catch {}
          }
        }
      } catch {
  ...
  const chartColor = isPositive ? "#22c55e" : "#3f3f46";
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          ...
          <div
            className={`text-sm font-medium font-mono mt-1 ${
              isPositive ? "text-[#22c55e]" : "text-[#3f3f46]"
            }`}
          >
            {isPositive ? "+" : "-"}
        ...
        <div className="flex items-center gap-2.5 self-start sm:self-center">
          {onOpenSend && (
            <button
              onClick={onOpenSend}
              className="px-4 py-2 rounded-xl bg-[#3f3f46] hover:bg-[#52525b] text-white text-xs font-semibold flex items-center gap-1.5 transition cursor-pointer"
            >
