  ...
  const [messages, setMessages] = useState<AssistantMessage[]>([
    {
      ...
      role: "assistant",
      content:
        "Avyro Protocol Assistant is active. I can parse transaction commands, check address poisoning, inspect daily guardrails, or summarize risks before you sign. What would you like to do?",
      timestamp: Date.now(),
    },
  ]);
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-full max-w-md bg-[#0c0c0c] border-l border-white/10 flex flex-col h-full shadow-2xl">
        ...
        <div className="p-4 border-b border-white/10 flex items-center justify-between">
          <h3 className="font-semibold text-white text-sm">Avyro Protocol Assistant</h3>
          <button
            type="button"
            onClick={onClose}
          ...
          >
            ...
            <div
              className={`relative inline-flex h-4 w-7 items-center rounded-full transition-colors ${
                engineMode === "smollm2_wasm" ? "bg-[#ffffff]" : "bg-white/20"
              }`}
            >
              <span
        ...
        {engineMode === "smollm2_wasm" && wasmProgress.status === "downloading" && (
          <div className="px-4 py-2 bg-[#ffffff]/5 border-b border-[#ffffff]/20 text-xs">
              ...
              {msg.role === "assistant" ? (
                <div className="relative group max-w-[90%] rounded-xl p-3 text-xs leading-relaxed bg-[#161616] text-white/90 border border-white/10">
                  <div className="flex items-center justify-between pb-1.5 mb-2 border-b border-white/5">
                    <span className="text-[10px] text-white/40 font-mono">Avyro Protocol</span>
        ...
        <div className="p-3 border-t border-white/10 bg-[#121212]">
          ...
          >
            <input
              ...
              className="flex-1 bg-black border border-white/10 rounded-lg px-3 py-2 text-xs text-white placeholder:text-white/30 focus:outline-none focus:border-[#ffffff] transition-colors"
            />
            <button
              type="submit"
              disabled={!inputText.trim() || isProcessing}
              className="p-2 rounded-lg bg-[#ffffff] text-white hover:bg-[#d4d4d4] disabled:opacity-40 disabled:hover:bg-[#ffffff] transition-colors"
            >
