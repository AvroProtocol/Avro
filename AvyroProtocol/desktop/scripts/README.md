# Verifying a Avyro inference receipt

Every reply from the Avyro Assistant carries a receipt: SHA-256 digests of
...
This tool checks that bundle without running, trusting, or contacting Avyro.

```sh
cd desktop
bun install
bun run verify:receipt path/to/avyro-inference-receipt-a1b2c3d4.json
```
