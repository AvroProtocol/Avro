...
stakingRouter.get("/v1/staking/config", (_req: Request, res: Response) => {
  res.status(200).json({
    ...
    tokenAddress: avyrTokenAddress,
    minStake: 10000,
    cycleDays: 30,
    tiers: [
      { name: "Tier 1", minAvyr: 10000, txnsPerMonth: 25 },
      { name: "Tier 2", minAvyr: 50000, txnsPerMonth: 100 },
      { name: "+50k Steps", minAvyr: 50000, txnsPerMonth: "+75 txns for each 50k" },
      { name: "Tier Unlimited", minAvyr: 1000000, txnsPerMonth: "Unlimited" },
    ],
  });
});
...
stakingRouter.post("/v1/staking/unstake", async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const unstakeWei = avyrDecimalToWei(numAmount);
