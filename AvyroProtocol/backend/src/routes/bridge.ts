...
bridgeRouter.get("/v1/bridge/rebates/:address", async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const avyrPrice = (await resolveAvyrPrice()).price;
    ...
    res.status(200).json({
      ...
      claimableAvyr:
        Number(avyrPrice) > 0 ? microsToAvyrWei(claimable, avyrPrice).toString() : null,
      avyrUsdPrice: Number(avyrPrice) > 0 ? avyrPrice : null,
...
bridgeRouter.post("/v1/bridge/rebates/:address/claim", async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const avyrWei = microsToAvyrWei(totalMicros, avyrPrice);
    const claim = await client.query(
      `INSERT INTO rebate_claims (
         user_address, amount_micros_usd, avyr_wei, avyr_usd_price, settlement_chain_id,
         avyr_price_source, avyr_reference_price, status
...
bridgeRouter.get("/v1/bridge/claims/:address", async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const result = await query(
      `SELECT id, amount_micros_usd, avyr_wei, avyr_usd_price, status, tx_hash,
    ...
    res.status(200).json({
      ...
      claims: result.rows.map((row) => ({
        ...
        avyrWei: row.avyr_wei,
        avyrUsdPrice: row.avyr_usd_price,
        status: row.status,
        txHash: row.tx_hash,
