...
export const STORAGE_KEY = "avyro.lang";
...
function translateInto(root: Element) {
  walkTextNodes(root, (node) => {
    ...
    let source = cached ?? current;

    if (cached !== undefined) {
      const cachedZh = lookup(cached);
      const currentKey = normalizeKey(current);
      if (cachedZh && currentKey === normalizeKey(cachedZh)) {
        source = cached;
      } else if (lookup(current)) {
        // React can update an existing text node in place (rotating hero copy,
        // interactive tabs, live state). Treat that new English value as the
        // latest source instead of forcing the stale translation back in.
        source = current;
...
function applyHeadLanguage(lang: Lang) {
  if (typeof document === "undefined") return;
  const description = document.querySelector('meta[name="description"]') as HTMLMetaElement | null;

  if (lang === "zh") {
    const currentTitle = document.title;
    if (lookup(currentTitle)) englishTitle = currentTitle;
    const titleSource = englishTitle || currentTitle;
    const zhTitle = lookup(titleSource);
    if (zhTitle) document.title = zhTitle;

    if (description) {
      const currentDescription = description.content;
      if (lookup(currentDescription)) englishDescription = currentDescription;
      const descriptionSource = englishDescription || currentDescription;
      const zhDescription = lookup(descriptionSource);
      if (zhDescription) description.content = zhDescription;
    }
    return;
  }

  if (englishTitle) document.title = englishTitle;
  if (description && englishDescription) description.content = englishDescription;
}
