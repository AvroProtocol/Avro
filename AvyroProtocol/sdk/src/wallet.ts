...
export class AvyroWallet {
  ...
  constructor(config: AvyroWalletConfig) {
  ...
  /**
   * Onboard and register a new 2-of-3 threshold smart account with the live AVYRO co-signer
   */
  static async create(options?: {
  ...
  }): Promise<{ wallet: AvyroWallet; shardC: LocalShard }> {
    ...
    const reg = (await response.json()) as AvyroWalletRegistration;
  ...
  /**
   * Combine two 65-byte signatures into the 130-byte payload expected by AvyroAccount.sol
   */
  ...
  async signUserOp(userOpHash: Hex): Promise<Hex> {
    ...
    return AvyroWallet.combineSignatures(sigA, sigB);
  }
  ...
  static async recoverWallet(options: RecoveryOptions): Promise<BundlerSubmissionResponse> {
    ...
    const signature = AvyroWallet.combineSignatures(sigC, signatureB);
