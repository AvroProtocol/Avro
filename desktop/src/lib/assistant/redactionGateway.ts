 ...
 *
 * Strictly enforces the "No Secrets to AI" rule from the Avyro Protocol V2 Privacy & AI Strategy Brief.
...
export function sanitizePromptIngress(prompt: string): RedactionResult {
  ...
  if (HEX_PRIVATE_KEY_REGEX.test(trimmed) || RAW_HEX_32_REGEX.test(trimmed)) {
    return {
      ...
      reason: "Private key or raw cryptographic secret detected. Avyro Protocol V2 strictly prohibits secrets from entering AI context.",
  ...
  if (tokens.length === 12 || tokens.length === 24) {
    ...
    if (isMnemonicCandidate) {
      return {
        ...
        reason: "Seed phrase detected. Avyro Protocol V2 strictly prohibits mnemonic words from entering AI context.",
  ...
  let sanitized = trimmed
    ...
    .replace(/(?:avyro|avyro)_[a-z0-9_]{10,}/gi, "[REDACTED_KEY_IDENTIFIER]");
