...
export async function processAssistantQuery(
  ...
  if (/^(hi|hello|hey|greetings|good\s+(morning|afternoon|evening))\b/i.test(lower)) {
    return await createResponse(
      "Hello! I am your Avyro Protocol Assistant. How can I assist you today? You can command me to prepare transfers, generate payment links, check address safety, or inspect spending guardrails."
    );
  }
  ...
  // What is Avyro Protocol
  if (/what\s+is\s+avyro(?:\s+protocol)?/i.test(lower)) {
    return await createResponse(
      "Avyro Protocol is an institutional-grade, non-custodial smart contract wallet on Robinhood Chain built with a 2-of-3 threshold shard architecture. Your device holds Shard A, the policy co-signer holds Shard B, and a passkey-backed recovery path protects Shard C. It features stealth transfers, spending guardrails, and address poisoning protection."
    );
  }
