...
import type { AvroWallet, UserPayLinkItem, PayLinkRouteMode } from "@avro/protocol-sdk";
import {
  createPayLink,
  listUserPayLinks,
  cancelPayLink,
...
export function PayLinksTab({ wallet, addToast, onBalanceRefresh }: PayLinksTabProps) {
  // Keep pay-link calls on the same co-signer origin as the active wallet.
  ...
  // Form states
  const [tokenSymbol, setTokenSymbol] = useState<"USDG" | "ETH" | "AVRO">("USDG");
  ...
  const handleCreateLink = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (tokenSymbol === "AVRO") tokenAddress = AVRO_TOKEN_ADDRESS;
  ...
  const handleCopyLink = (slug: string) => {
    const url = `https://avroprotocol.com/pay/${slug}`;
                ...
                <div className="mt-4 rounded-xl bg-[#0e121b] border border-[#1e293b] p-4 flex flex-col sm:flex-row items-center gap-4">
                  ...
                  <div className="min-w-0 flex-1 text-center sm:text-left space-y-2">
                    ...
                    <div className="font-mono text-[11px] text-slate-400 break-all">
                      https://avroprotocol.com/pay/{link.slug}
                    </div>
            ...
            <form onSubmit={handleCreateLink} className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(AVRO_TOKEN_CONFIGURED ? (["USDG", "ETH", "AVRO"] as const) : (["USDG", "ETH"] as const)).map((sym) => (
                    <button
                      key={sym}
                      type="button"
