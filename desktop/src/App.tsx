...
export function App() {
  const [desktopTheme, setDesktopTheme] = useState<"dark" | "light">(() => {
    try {
      return localStorage.getItem("avro.desktop.theme") === "light" ? "light" : "dark";
    } catch {
      return "dark";
    }
  });

  const toggleDesktopTheme = useCallback(() => {
    setDesktopTheme((current) => {
      const next = current === "dark" ? "light" : "dark";
      try { localStorage.setItem("avro.desktop.theme", next); } catch {}
      return next;
    });
  }, []);
  ...
  const [accounts, setAccounts] = useState<WalletAccount[]>(() => {
    const activeWalletAddr = typeof window !== "undefined" ? localStorage.getItem("avro_wallet_address") : null;
    const initialAddr = (activeWalletAddr as Address) || ("0x0000000000000000000000000000000000000000" as Address);
    try {
      const stored = localStorage.getItem("avro_accounts");
      if (stored) {
  ...
  const [wallet, setWallet] = useState<AvroWallet | null>(null);
  ...
  useEffect(() => {
    if (walletAddress && walletAddress !== "0x0000000000000000000000000000000000000000") {
      setAccounts((prev) => {
        ...
        if (changed) {
          try {
            localStorage.setItem("avro_accounts", JSON.stringify(updated));
          } catch {}
          return updated;
        }
        return prev;
  ...
  const [ethPrice, setEthPrice] = useState<number>(() => {
    try {
      const cached = localStorage.getItem("avro_eth_price");
  ...
  const [eth24hChange, setEth24hChange] = useState<number>(() => {
    try {
      const cached = localStorage.getItem("avro_eth_24h_change");
  ...
  const [lastPriceFetchTime, setLastPriceFetchTime] = useState<number>(() => {
    try {
      const cached = localStorage.getItem("avro_eth_price_time");
  ...
  const [totpVerified, setTotpVerified] = useState<boolean>(() => {
    try {
      const activeId = localStorage.getItem("avro_active_account_id") || "primary";
      const activeAddr = localStorage.getItem(`avro_wallet_address_${activeId}`) || localStorage.getItem("avro_wallet_address");
      if (activeAddr && localStorage.getItem(`avro_totp_enrolled_${activeAddr.toLowerCase()}`) === "true") {
        return true;
      }
      if (localStorage.getItem(`avro_totp_enrolled_${activeId}`) === "true") {
        return true;
      }
      if (activeId === "primary" && localStorage.getItem("avro_totp_enrolled") === "true") {
        return true;
      }
      return false;
    } catch {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (typeof p === "number" && p > 0) {
          ...
          try {
            localStorage.setItem("avro_eth_price", String(p));
            localStorage.setItem("avro_eth_price_time", String(now));
          } catch {}
        }
        if (typeof change === "number") {
          setEth24hChange(change);
          try {
            localStorage.setItem("avro_eth_24h_change", String(change));
          } catch {}
        }
        return p;
      }
  ...
  useEffect(() => {
    ...
    try {
      const key = `avro_transactions_${walletAddress.toLowerCase()}`;
  ...
  useEffect(() => {
    ...
    async function loadSavedWallet() {
      try {
        ...
        let savedAddress = localStorage.getItem(`avro_wallet_address_${activeId}`) || (activeId === "primary" ? localStorage.getItem("avro_wallet_address") : null);

        let savedShardA: string | null = null;
        if (savedAddress) {
          savedShardA = localStorage.getItem(`avro_shard_a_${savedAddress.toLowerCase()}`);
        }
        if (!savedShardA) {
          savedShardA = localStorage.getItem(`avro_shard_a_${activeId}`) || (activeId === "primary" ? localStorage.getItem("avro_shard_a") : null);
        }
        ...
        if (savedShardA) {
          try {
            ...
            if (savedAddress && derived.toLowerCase() !== savedAddress.toLowerCase()) {
              console.warn(`[Avro Protocol] Key alignment: loaded shard A derives ${derived} while saved address was ${savedAddress}. Synchronizing active wallet address.`);
              savedAddress = derived;
            }
          } catch {}
        }

        const savedApiKey = localStorage.getItem(`avro_api_key_${activeId}`) || (activeId === "primary" ? localStorage.getItem("avro_api_key") : null);
        const savedShardB = localStorage.getItem(`avro_shard_b_address_${activeId}`) || (activeId === "primary" ? localStorage.getItem("avro_shard_b_address") : null);
        const savedShardC = localStorage.getItem(`avro_shard_c_address_${activeId}`) || (activeId === "primary" ? localStorage.getItem("avro_shard_c_address") : null);
        ...
        if (savedShardA && savedAddress && savedApiKey && savedShardB && savedShardC) {
          ...
          const restoredWallet = new AvroWallet({
          ...
          localStorage.removeItem("avro_shard_c_key");

          const isTotp =
            localStorage.getItem(`avro_totp_enrolled_${savedAddress.toLowerCase()}`) === "true" ||
            localStorage.getItem(`avro_totp_enrolled_${activeId}`) === "true" ||
            (activeId === "primary" && localStorage.getItem("avro_totp_enrolled") === "true");
  ...
  const handleSelectAccount = async (targetId: string) => {
    ...
    try {
      localStorage.setItem("avro_active_account_id", targetId);
    } catch {}

    const savedAddress = localStorage.getItem(`avro_wallet_address_${targetId}`) || (targetId === "primary" ? localStorage.getItem("avro_wallet_address") : null);
    ...
    if (!savedShardA) {
      savedShardA = localStorage.getItem(`avro_shard_a_${targetId}`) || (targetId === "primary" ? localStorage.getItem("avro_shard_a") : null);
    }
    if (!savedShardA && isTauri()) {
      if (savedAddress) {
        try {
    ...
    const savedApiKey = localStorage.getItem(`avro_api_key_${targetId}`) || (targetId === "primary" ? localStorage.getItem("avro_api_key") : null);
    const savedShardB = localStorage.getItem(`avro_shard_b_address_${targetId}`) || (targetId === "primary" ? localStorage.getItem("avro_shard_b_address") : null);
    const savedShardC = localStorage.getItem(`avro_shard_c_address_${targetId}`) || (targetId === "primary" ? localStorage.getItem("avro_shard_c_address") : null);
    ...
    if (savedShardA && savedAddress && savedApiKey && savedShardB && savedShardC) {
      ...
      const isTotp =
        localStorage.getItem(`avro_totp_enrolled_${savedAddress.toLowerCase()}`) === "true" ||
        localStorage.getItem(`avro_totp_enrolled_${targetId}`) === "true" ||
        (targetId === "primary" && localStorage.getItem("avro_totp_enrolled") === "true");
      setTotpVerified(isTotp);
      setTotpSecret(null);
      setTotpQrCode(null);
      setTotpCode("");
  ...
  const handleConfirmRemoveAccount = async (target: WalletAccount) => {
    try {
      ...
      localStorage.removeItem(`avro_wallet_address_${target.id}`);
      localStorage.removeItem(`avro_shard_a_${target.id}`);
      if (cleanAddress) {
        localStorage.removeItem(`avro_shard_a_${cleanAddress}`);
        localStorage.removeItem(`avro_totp_enrolled_${cleanAddress}`);
        localStorage.removeItem(`avro_transactions_${cleanAddress}`);
      }
      localStorage.removeItem(`avro_shard_b_address_${target.id}`);
      localStorage.removeItem(`avro_shard_c_address_${target.id}`);
      localStorage.removeItem(`avro_api_key_${target.id}`);
      localStorage.removeItem(`avro_totp_enrolled_${target.id}`);

      if (target.id === "primary") {
        localStorage.removeItem("avro_wallet_address");
        localStorage.removeItem("avro_shard_a");
        localStorage.removeItem("avro_shard_b_address");
        localStorage.removeItem("avro_shard_c_address");
        localStorage.removeItem("avro_api_key");
        localStorage.removeItem("avro_totp_enrolled");
      }
      ...
      if (remaining.length === 0) {
        ...
        setTransactions([]);
        try {
          localStorage.removeItem("avro_accounts");
          localStorage.removeItem("avro_active_account_id");
        } catch {}
        addToast("info", "Wallet Removed", `${displayName} was removed. No wallets remaining on this device.`);
      } else {
        setAccounts(remaining);
        try {
          localStorage.setItem("avro_accounts", JSON.stringify(remaining));
        } catch {}
  ...
  const handleCreateWallet = async () => {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        const { wallet: newWallet, shardC } = await AvroWallet.create();
        ...
        localStorage.setItem(`avro_shard_a_${newAccId}`, newWallet.shardA.privateKey);
        localStorage.setItem(`avro_shard_a_${newWallet.address.toLowerCase()}`, newWallet.shardA.privateKey);
        if (newAccId === "primary") {
          localStorage.setItem("avro_shard_a", newWallet.shardA.privateKey);
        }

        // Save account-specific credentials
        localStorage.setItem(`avro_wallet_address_${newAccId}`, newWallet.address);
        localStorage.setItem(`avro_api_key_${newAccId}`, newWallet.apiKey);
        localStorage.setItem(`avro_shard_b_address_${newAccId}`, newWallet.shardB.address);
        localStorage.setItem(`avro_shard_c_address_${newAccId}`, shardC.address);

        if (newAccId === "primary") {
          localStorage.setItem("avro_wallet_address", newWallet.address);
          localStorage.setItem("avro_api_key", newWallet.apiKey);
          localStorage.setItem("avro_shard_b_address", newWallet.shardB.address);
          localStorage.setItem("avro_shard_c_address", shardC.address);
        }
        ...
        try {
          localStorage.setItem("avro_accounts", JSON.stringify(updatedAccounts));
          localStorage.setItem("avro_active_account_id", newAccId);
        } catch {}
        ...
        try {
          localStorage.setItem(`avro_totp_enrolled_${newAccId}`, "false");
          localStorage.setItem(`avro_totp_enrolled_${newWallet.address.toLowerCase()}`, "false");
          if (newAccId === "primary") {
            localStorage.setItem("avro_totp_enrolled", "false");
          }
          localStorage.setItem(`avro_transactions_${newWallet.address.toLowerCase()}`, JSON.stringify([]));
        } catch {}
  ...
  const handleImportPrivateKey = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem(`avro_shard_a_${accId}`, cleanKey);
      localStorage.setItem(`avro_shard_a_${cleanAddress}`, cleanKey);
      localStorage.setItem(`avro_wallet_address_${accId}`, cleanAddress);
      if (accId === "primary") {
        localStorage.setItem("avro_shard_a", cleanKey);
        localStorage.setItem("avro_wallet_address", cleanAddress);
      }
      ...
      try {
        ...
        if (infoRes.ok) {
          ...
          localStorage.setItem(`avro_shard_b_address_${accId}`, shardBAddress);
          localStorage.setItem(`avro_shard_c_address_${accId}`, shardCAddr);
          localStorage.setItem(`avro_api_key_${accId}`, apiKeyVal);
        }
      } catch {}

      const shardB = new RemoteCosigner(shardBAddress, cleanAddress, DEFAULT_API_URL);
      const importedWallet = new AvroWallet({
      ...
      setAccounts((prev) => {
        ...
        try {
          localStorage.setItem("avro_accounts", JSON.stringify(updated));
          localStorage.setItem("avro_active_account_id", accId);
        } catch {}
        return updated;
      });
  ...
  const handleRecoverWallet = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        await AvroWallet.recoverWallet({
      ...
      localStorage.setItem(`avro_shard_a_${accId}`, newShardA.privateKey);
      localStorage.setItem(`avro_shard_a_${cleanAddress}`, newShardA.privateKey);
      localStorage.setItem(`avro_wallet_address_${accId}`, cleanAddress);
      localStorage.setItem(`avro_shard_c_address_${accId}`, shardC.address);
      localStorage.setItem("avro_totp_enrolled", "true");
      if (accId === "primary") {
        localStorage.setItem("avro_shard_a", newShardA.privateKey);
        localStorage.setItem("avro_wallet_address", cleanAddress);
        localStorage.setItem("avro_shard_c_address", shardC.address);
      }

      // 4. Fetch wallet info from cosigner to construct AvroWallet
      let shardBAddress: Address = "0x0000000000000000000000000000000000000000";
      let apiKeyVal = "";
      try {
        const infoRes = await fetch(`${DEFAULT_API_URL}/v1/wallets/${cleanAddress}`);
        if (infoRes.ok) {
          ...
          localStorage.setItem(`avro_api_key_${accId}`, apiKeyVal);
          if (accId === "primary") {
            localStorage.setItem("avro_shard_b_address", shardBAddress);
            localStorage.setItem("avro_api_key", apiKeyVal);
          }
        }
      } catch {}
  ...
  const handleSendTransaction = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (isStealthSend && isMeta) {
        ...
        if (wallet?.address) {
          try {
            localStorage.setItem(`avro_transactions_${wallet.address.toLowerCase()}`, JSON.stringify(updatedList));
          } catch {}
      ...
      const signature = await timeStage("combine", onStage, async () =>
        AvroWallet.combineSignatures(sigA, sigB)
      );
  ...
  const handleConfirmTotp = async () => {
    ...
    try {
      ...
      if (success) {
        setTotpVerified(true);
        try {
          localStorage.setItem(`avro_totp_enrolled_${activeAccountId}`, "true");
          if (wallet.address) {
            localStorage.setItem(`avro_totp_enrolled_${wallet.address.toLowerCase()}`, "true");
          }
  ...
  return (
    <div className={`min-h-screen h-screen flex flex-col bg-[#050505] text-slate-100 font-sans overflow-hidden selection:bg-white/20 ${desktopTheme === "light" ? "avro-desktop-light" : "avro-desktop-dark"}`}>
      {/* 1.5s Splash Screen with Avro Protocol Logo */}
      {showSplash && (
        <div className="fixed inset-0 z-50 bg-[#050505] flex flex-col items-center justify-center select-none">
      ...
      {!wallet ? (
        <div className="flex-1 flex flex-col items-center justify-center p-6 text-center max-w-xl mx-auto w-full space-y-7 animate-in fade-in duration-500 overflow-y-auto select-none relative">
          <button
            type="button"
            onClick={toggleDesktopTheme}
            className="absolute top-5 right-5 w-10 h-10 rounded-xl border border-white/10 bg-white/[0.04] hover:bg-white/[0.09] text-slate-300 hover:text-white transition flex items-center justify-center"
            title={desktopTheme === "dark" ? "Switch to light theme" : "Switch to dark theme"}
            aria-label={desktopTheme === "dark" ? "Switch to light theme" : "Switch to dark theme"}
          >
            {desktopTheme === "dark" ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
          </button>
          {/* Logo & Headline */}
          <div className="flex flex-col items-center space-y-4">
            <div className="w-20 h-20 rounded-2xl bg-white/[0.04] border border-white/10 p-3.5 flex items-center justify-center shadow-2xl">
              <img src="/logo.png" alt="Avro Protocol" className="w-full h-full object-contain" />
            </div>
            <div>
              <h1 className="text-3xl font-semibold tracking-tight text-white">Avro Protocol Self-Custody</h1>
          ...
          <div className="w-full max-w-sm">
            <button
              ...
              disabled={isSending}
              className="w-full py-3.5 px-6 rounded-xl bg-[#3f3f46] hover:bg-[#52525b] text-white font-semibold text-sm transition flex items-center justify-center gap-2.5 shadow-xl disabled:opacity-50 cursor-pointer"
            >
              {isSending ? (
                <>
          ...
          <div className="w-full space-y-2.5 pt-2 text-left">
            ...
            <div className="rounded-xl bg-[#181a22] border border-white/[0.08] overflow-hidden">
              ...
              >
                <div className="text-xs font-semibold text-white flex items-center gap-2">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#3f3f46]"></span>
                  <span>How is Avro Protocol different from regular wallets?</span>
                </div>
                <ChevronDown
                  className={`w-3.5 h-3.5 text-slate-400 transition-transform duration-200 shrink-0 ${
              ...
              {openFaqIndex === 1 && (
                <div className="px-4 pb-4 text-xs text-slate-400 leading-relaxed pl-7.5 border-t border-white/5 pt-2.5">
                  Regular wallets rely on a single, vulnerable 12-word seed phrase that presents a single point of failure. Avro Protocol splits protection into a 2-of-3 threshold quorum: your local device key (Shard A), an automated co-signer (Shard B), and an emergency backup (Shard C) protected by 2FA. No single key can ever steal your assets.
                </div>
              )}
            </div>
          </div>
          ...
          <div className="pt-2 flex items-center justify-center gap-6 text-xs text-slate-500">
            <a
              href="https://avroprotocol.com"
              target="_blank"
              rel="noopener noreferrer"
              className="hover:text-slate-300 transition flex items-center gap-1.5"
            >
              <span>Avro Protocol Web</span>
              <ExternalLink className="w-3 h-3" />
            </a>
            <span className="text-slate-700">•</span>
            <a
              href="https://x.com/Avroproto"
              target="_blank"
              rel="noopener noreferrer"
              className="hover:text-slate-300 transition flex items-center gap-1.5"
            >
              <span>Avro Protocol on X</span>
              <ExternalLink className="w-3 h-3" />
            </a>
          </div>
        </div>
      ...
      {/* Main Canvas */}
      <main className="flex-1 flex flex-col h-full overflow-y-auto overflow-x-hidden bg-[#050505]">
        ...
        <header className="flex items-center justify-between px-8 py-3.5 border-b border-white/[0.06] shrink-0">
          ...
          <div className="flex items-center gap-4 text-xs">
            ...
            {wallet && (
              <button
                ...
                title={isGaslessActive ? "100% Gasless Active (Protocol Sponsored)" : "Stake $AVRO to Go Gasless"}
              >
          ...
          <div className="flex items-center gap-2.5">
            <button
              type="button"
              onClick={toggleDesktopTheme}
              className="h-8 w-8 rounded-[10px] bg-white/[0.04] hover:bg-white/[0.09] text-slate-300 border border-white/[0.06] transition flex items-center justify-center cursor-pointer"
              title={desktopTheme === "dark" ? "Switch to light theme" : "Switch to dark theme"}
              aria-label={desktopTheme === "dark" ? "Switch to light theme" : "Switch to dark theme"}
            >
              {desktopTheme === "dark" ? <Sun className="w-3.5 h-3.5" /> : <Moon className="w-3.5 h-3.5" />}
            </button>
            ...
            {!wallet && (
              <button
                ...
                className="h-8 px-4 rounded-xl text-xs font-semibold bg-[#3f3f46] hover:bg-[#52525b] text-white transition flex items-center gap-1.5 cursor-pointer"
              >
              ...
              <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/20 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg bg-[#3f3f46] hover:bg-[#52525b] text-xs font-medium text-white transition"
                  >
                    Set Up 2FA
                  </button>
                </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 pb-2">
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="px-5 py-2.5 rounded-xl bg-[#3f3f46] hover:bg-[#52525b] text-white font-medium text-xs flex items-center gap-2 transition disabled:opacity-40"
                  >
        ...
        {activeTab === "tokens" && (
          <div className="p-8 max-w-4xl space-y-6">
            <div className="flex items-center justify-between pb-2">
              <div>
                <div className="flex items-center gap-2.5">
                  <Coins className="w-5 h-5 text-[#3f3f46]" />
              ...
              <button
                onClick={() => openSendModal()}
                disabled={!wallet}
                className="px-4 py-2 rounded-xl bg-[#3f3f46] hover:bg-[#52525b] text-white text-xs font-medium transition flex items-center gap-1.5 disabled:opacity-40"
              >
            ...
            <div className="p-4 rounded-xl bg-[#181a22] border border-white/[0.08] flex items-center justify-between">
              <div className="flex items-center gap-3.5">
                ...
                <div>
                  ...
                  <div className="text-xs text-slate-400 mt-0.5 font-mono">
                    {wallet ? `Co-signer: ${shortenAddress(wallet.shardB.address)} (api.avroprotocol.com)` : "api.avroprotocol.com"}
                  </div>
                </div>
              </div>
              <span className="text-xs font-medium px-2.5 py-1 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 flex items-center gap-1.5">
            ...
            <div className="p-6 rounded-2xl bg-[#181a22] border border-white/[0.08] backdrop-blur-sm space-y-4">
              ...
              {totpVerified ? (
                <div className="space-y-4 pt-2">
                  ...
                  <div className="p-4 rounded-xl bg-black/40 border border-white/10 space-y-3">
                    <div className="flex items-start gap-3">
                      <ShieldAlert className="w-4 h-4 text-[#3f3f46] shrink-0 mt-0.5" />
                      <div>
                ...
                <button
                  ...
                  className="px-5 py-2.5 rounded-xl bg-[#3f3f46] hover:bg-[#52525b] text-white font-medium text-xs transition disabled:opacity-40"
                >
                ...
                <div className="space-y-4 pt-3 border-t border-white/10">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="px-5 py-2 rounded-xl bg-[#3f3f46] hover:bg-[#52525b] text-white font-medium text-xs transition disabled:opacity-40 flex items-center gap-1.5"
                    >
                      {isConfirmingTotp ? (
                        <>
        ...
        {/* Tab: Stake $AVRO to Go Gasless (v0.1.7) */}
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <div className="bg-[#181a23] border border-white/15 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Wallet className="w-5 h-5 text-[#3f3f46]" />
            ...
            <div className="space-y-3 pt-1">
              <button
                ...
                disabled={isSending}
                onClick={handleCreateWallet}
                className="w-full p-4 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 hover:border-[#3f3f46]/50 transition text-left flex items-start gap-3.5 disabled:opacity-50 group cursor-pointer"
              >
                <div className="w-10 h-10 rounded-xl bg-[#3f3f46]/10 border border-[#3f3f46]/20 flex items-center justify-center shrink-0 text-[#3f3f46] group-hover:scale-105 transition-transform">
                  {isSending ? (
                    <Loader2 className="w-4 h-4 animate-spin" />
                  ) : (
                    <PlusCircle className="w-4 h-4" />
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold text-white group-hover:text-[#3f3f46] transition-colors">
                ...
                <form onSubmit={handlePreviewTransfer} className="space-y-4">
                  ...
                  <div className="flex gap-2 pt-2">
                    ...
                    <button
                      ...
                      className="flex-1 py-2.5 rounded-xl bg-[#3f3f46] hover:bg-[#52525b] text-white font-semibold text-xs transition disabled:opacity-40 flex items-center justify-center gap-2"
                    >
              ...
              <>
                    ...
                    >
                      ...
                      <label className="flex items-start gap-2 cursor-pointer select-none">
                        <input
                          ...
                          className="mt-0.5 w-3.5 h-3.5 shrink-0 accent-[#3f3f46] cursor-pointer"
                        />
                ...
                {isSimulating ? (
                  <div className="p-3.5 rounded-xl bg-white/[0.03] border border-white/[0.08] flex items-center gap-3">
                    <Loader2 className="w-4 h-4 text-[#3f3f46] animate-spin shrink-0" />
                    <div className="text-xs">
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <div className="bg-[#181a23] border border-white/15 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl">
            ...
            <div className="pt-2">
              <button
                onClick={() => setShowReceiveModal(false)}
                className="w-full py-2.5 rounded-xl bg-[#3f3f46] hover:bg-[#52525b] text-white font-medium text-xs transition"
              >
                Done
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <div className="bg-[#181a23] border border-white/15 rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-2xl">
            ...
            <div className="p-3.5 rounded-xl bg-white/[0.03] border border-white/10 flex items-start gap-3">
              <Fingerprint className="w-5 h-5 text-[#3f3f46] shrink-0 mt-0.5" />
              <div className="text-xs">
            ...
            <div className="flex gap-2 pt-2">
              ...
              <button
                ...
                className="flex-1 py-2.5 rounded-xl bg-[#3f3f46] hover:bg-[#52525b] text-white font-semibold text-xs transition flex items-center justify-center gap-1.5"
              >
      ...
      {showRecoverModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <div className="bg-[#181a23] border border-white/15 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <KeyRound className="w-5 h-5 text-[#3f3f46]" />
                ...
                <form onSubmit={handleImportPrivateKey} className="space-y-3.5">
                  ...
                  <div className="pt-2">
                    <button
                      ...
                      className="w-full py-2.5 rounded-xl bg-[#3f3f46] hover:bg-[#52525b] text-white font-semibold text-xs transition disabled:opacity-40 flex items-center justify-center gap-2 cursor-pointer"
                    >
                      {isRecovering ? (
                        <>
                          <Loader2 className="w-3.5 h-3.5 animate-spin" />
          ...
          <div className="bg-[#181a22] border border-white/10 rounded-2xl p-6 max-w-md w-full shadow-2xl space-y-4">
            ...
            <div className="flex gap-2 pt-2">
              ...
              <button
                ...
                className="flex-1 py-2.5 rounded-xl bg-[#3f3f46] hover:bg-[#52525b] text-white text-xs font-semibold transition cursor-pointer"
              >
                Remove Wallet
              </button>
            </div>
      ...
      {/* Avro Protocol Assistant Widget (V2 On-Device AI) */}
      <AssistantWidget
