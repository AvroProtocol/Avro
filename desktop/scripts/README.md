# Verifying a Avro inference receipt

Every reply from the Avro Assistant carries a receipt: SHA-256 digests of
...
This tool checks that bundle without running, trusting, or contacting Avro.

```sh
cd desktop
bun install
bun run verify:receipt path/to/avro-inference-receipt-a1b2c3d4.json
```
