...
export async function freezeMobileWallet(
  ...
  const cleanAddr = walletAddress.trim().toLowerCase();
  const apiKey = await loadEncryptedItem(`avro_apikey_${cleanAddr}`);
...
export async function setupMobileTotp(
  ...
  const data = await res.json();
  await saveEncryptedItem(`avro_totp_pending_${cleanAddr}`, data.secret);
  return {
    secret: data.secret,
    uri: data.uri,
  };
...
export async function confirmMobileTotp(
  ...
  const data = await res.json();
  if (data.ok) {
    await saveEncryptedItem(`avro_totp_enabled_${cleanAddr}`, "true");
    return true;
  }
  return false;
}
